...
# The golden image can contain Zed's last workspace, including editor buffers,
# tabs, panes, selections and scroll positions.  They are all workspace-owned
# in this schema; deleting only one editor path leaves the other restore state
# capable of bringing an unrelated buffer back into the parity capture.
tart exec "$RUN" bash -lc '
  pkill -x Zed 2>/dev/null || true
  pkill -x zed 2>/dev/null || true
  db="/Users/admin/Library/Application Support/Zed/db/0-stable/db.sqlite"
  if [ -f "$db" ]; then
    sqlite3 "$db" "PRAGMA foreign_keys=ON; BEGIN IMMEDIATE; DELETE FROM workspaces; COMMIT;"
  fi
' || exit 1
...
case "$MODE" in
  ...
  smoke|regression|parity|profile|selection|emoji|blame)
    ...
    for p in nimculus-emoji.png zed-emoji.png nimculus-selection.png zed-selection.png nimculus-inline-blame.png nimculus-inline-blame-padding-0.png nimculus-inline-blame-padding-4.png nimculus-inline-blame-padding-12.png nimculus-inline-blame-padding-20.png zed-inline-blame.png hot-lines.txt sample-Nimculus.txt sample-zed.txt nimculus-window.png zed-window.png nimculus-workspace.png zed-workspace.png nimculus-ms-per-scroll.txt zed-ms-per-scroll.txt scroll-normalised.txt nimculus-scroll-cal-0.png nimculus-scroll-cal-1.png nimculus-scroll-cal-2.png nimculus-scroll-cal-4.png nimculus-scroll-cal-8.png nimculus-scroll-cal-16.png zed-scroll-cal-0.png zed-scroll-cal-1.png zed-scroll-cal-2.png zed-scroll-cal-4.png zed-scroll-cal-8.png zed-scroll-cal-16.png \
