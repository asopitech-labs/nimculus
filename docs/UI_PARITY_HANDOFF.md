...
最終更新: 2026-08-07 / ブランチ `main`（`a182b00` まで）
...
`a182b00` 時点の実測。条件は §2 のとおり揃えてある:

```
overall identical 73.22%   diff<=2 89.98%   diff<=32 92.86%   >32 7.14%   mean 8.07
titlebar   37.47% / >32 3.69%   (0-34pt)
tabbar     35.95% / >32 5.85%   (35-66pt)
toolbar    33.14% / >32 9.63%   (67-112pt)
editor     80.15% / >32 6.96%   (112-745pt)
seam       26.91% / >32 24.02%  (745-761pt)
...
**Lilex の同梱は一度試して差し戻した**（`a182b00` のコミットメッセージ参照）。
