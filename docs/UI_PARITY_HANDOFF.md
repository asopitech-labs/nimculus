...
最終更新: 2026-08-07 / ブランチ `main`（`e661fff` まで）
...
`e661fff`（シーム修正）時点の実測。条件は §2 のとおり揃えてある:

```
overall identical 72.41%   diff<=2 88.86%   diff<=32 92.73%   >32 7.27%   mean 8.38
titlebar   37.47% / >32 3.69%   (0-34pt)
tabbar     31.30% / >32 5.88%   (35-66pt)
toolbar    33.14% / >32 9.63%   (67-112pt)
editor     80.06% / >32 7.10%   (112-745pt)
seam       26.47% / >32 24.45%  (745-761pt)
...
（§2「どの数値で判定するか」）。tabbar の 35.95% → 31.30% はディザのパターン差で、
実体は ±1 の色差。
...
1. **水平スクロールバーの実装**（シーム帯 745–760pt、>32 が 24.45% と最悪）。
   `e661fff` で罫線を入れ、帯をガターとドックから外したが、**サムの長さは未対応**。
   Zed は 92–1134pt のトラックに 92–915pt のサムを描く。こちらは
   `NimculusStatusBandOverlay` がテキスト列の全幅（84–1166pt）を塗ったままの暫定。
   `main.nim` の `horizontalEditorScrollbar` が本来の実装なので、そちらに寄せる。
   併せて確認すること: Zed はソフトラップが効いていても（5 行目が折り返している）
   水平スクロールバーを出している。この条件を先に理解しないと、サムの長さは決まらない。
   なお `editorGap` を 745pt より下へ伸ばすと**本文の行を塗り潰す**（不透明な AppKit
   ビューのため）。伸ばさないこと。
2. **ステータスバーがドックの下に伸びていない**。Zed は 761–791pt の `#dcddde` が
   全幅。Nimculus は x 1166pt までで、その先はドックの `#ececed` のまま。
3. **ツールバー/パンくず**（>32 9.63%）。帯の位置・罫線・テキスト上端
   （82.0pt）は一致済み。残差はグリフそのもの。マーカー（`#`/`##`）の淡色具合と
   見出しの太さを実ピクセルで比較する。
4. **タブバー**。アクティブタブの面は Zed が x 61.0–314.5pt、
   Nimculus が 60.0–299.5pt。左端 1pt・幅 14pt の差なので、まずラベルの
   フォントメトリクスと省略規則を突き合わせる。Zed は 2 タブ目が斜体＋グレー背景、
   未保存は青ドット。
5. **タイトルバー**。ただし Zed 側に "Restart to Update" / "Sign In" /
