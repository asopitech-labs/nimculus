...
`DESIGN_DECISIONS.md` の 154 件と突き合わせて済みを消し込んだもの。
**この文書が移植の台帳。** 番号は表題で照合して解決済み（2026-08-10 の採番し直しを反映）。
...
対応表と再発防止の採番台帳は `DESIGN_DECISIONS.md` の冒頭にある。

## 2. 層ごとの残作業

**「一部のみ」を先に片付ける。** 動いているように見えて条件を変えると壊れるため、
新しい機能を足すより優先度が高い。詳細と Nim での再現方式は
[`ZED_ARCHITECTURE.md`](ZED_ARCHITECTURE.md) の同名の節にある。
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-069 | Convert logical top-origin frames at the AppKit boundary | 実装済み |
| UI-111 | 最大の構造差 — chrome を AppKit で描いていること | 却下 |
...
- **Menu bar construction from a declarative menu tree with keystrokes from the keymap** — Zed `crates/gpui_macos/src/platform.rs:241 (create_menu_bar), :305 (create_menu_item), :1387 (handle_menu_item), :1404 (validate_menu_item)`
- **Accessibility tree** — Zed `crates/gpui_macos/src/window.rs:1862 (a11y_init), :1881 (a11y_tree_update), :1893 (a11y_update_window_bounds), :535 (accesskit_macos::SubclassingAdapter field), :1898-1911 (activation and action handlers)`

**無い**

- **The Platform trait as the OS boundary** — Zed `crates/gpui_macos/src/platform.rs:475 (impl Platform for MacPlatform)`
- **Per-window state behind a handle stored in an ObjC ivar** — Zed `crates/gpui_macos/src/window.rs:77 (WINDOW_STATE_IVAR), :490 (MacWindowState), :1959 (get_window_state), :1969 (drop_window_state)`
- **Traffic-light repositioning for an app-drawn titlebar** — Zed `crates/gpui_macos/src/window.rs:541 (move_traffic_light), :601 (capture_traffic_light_frames), :638 (restore_traffic_light)`
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-101 | Port Zed's accessibility tree through NimNUI and NSAccessibility | 実装済み |
| UI-102 | Port Zed's async execution on Nim's own Future | 実装済み |
| UI-103 | Extend the layout spec toward Zed's Style, without adopting a layout engine | 実装済み |
...
- **Effect queue + flush_effects re-entrancy guard** — Zed `crates/gpui/src/app.rs:1516 push_effect, :1531 flush_effects, :1029 App::update, :1036 start_update, :1040 finish_update`
- **Double-buffered Frame with element-state carryover** — Zed `crates/gpui/src/window.rs:824 struct Frame, :867 Frame::new, :884 Frame::clear, :965 Frame::finish, :1015-1016 rendered_frame/next_frame fields, :2737-2741 mem::swap in draw`
- **Per-element retained state keyed by GlobalElementId** — Zed `crates/gpui/src/window.rs:3562 with_element_state, :3517 use_keyed_state, :3546 use_state, :3505 with_element_namespace, :6157 enum ElementId`
- **Window/App split: root view, viewport, scale factor, refresh** — Zed `crates/gpui/src/window.rs:989 struct Window (root :1006, viewport_size :1004, layout_engine :1005, scale_factor :1029), :2347 viewport_size(), :2460 scale_factor(), :2466 rem_size(), app.rs:711 windows SlotMap, app.rs:1217 open_window, app.rs:1728 update_window_id`
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-100 | Port Zed's text-layout layers without moving document state into platform | 実装済み |
| UI-105 | Rounded selection first, as a shaped primitive rather than a path engine | 実装済み |
| UI-106 | Font features and fallbacks as CTFont attributes | 実装済み |
| UI-109 | Zed の Primitive は「形」で、Nimculus の PaintKind は「意味」で分かれている | 却下 |
| UI-110 | PolychromeSprite を入れて、まずカラー絵文字を描く | 実装済み |
...
- **DecorationRun coalescing (style runs separated from font runs)** — Zed `crates/gpui/src/text_system.rs:409-427 (shape_line), :537-567 (shape_text), crates/gpui/src/text_system/line.rs:24 (DecorationRun)`
...
- **Scene primitive set and per-kind streams** — Zed `crates/gpui/src/scene.rs:41 (Scene), :222 (Primitive), :501 (Quad), :521 (Underline), :540 (Shadow), :677 (MonochromeSprite), :696 (SubpixelSprite), :715 (PolychromeSprite), :755 (Path), :734 (PaintSurface)`
- **Pixels -> ScaledPixels boundary and device-pixel snapping** — Zed `crates/gpui/src/window.rs:3886 (round_to_device_pixel), :3952 (origin.scale(scale_factor)), :3964 (integer origin), scene.rs:787 (Path::scale)`
...
- **Background: solid vs gradient vs pattern as one shader-visible tag** — Zed `crates/gpui/src/color.rs:779 (Background), :759 (ColorSpace), :851 (solid_background), :865 (linear_gradient), :827 (pattern_slash), :841 (checkerboard); used by scene.rs:506 (Quad.background) and :761 (Path.color)`
...
- **UnderlineStyle / StrikethroughStyle and their snapped emission** — Zed `crates/gpui/src/style.rs:824 (UnderlineStyle), :839 (StrikethroughStyle); crates/gpui/src/window.rs:3870 (paint_underline), :3905 (paint_strikethrough)`
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-005 | One focused pane defines the complete macOS IME document context | 実装済み |
| UI-104 | Port Zed's ongoing-scroll axis lock, not its touch gestures | 実装済み |
| UI-107 | Context-dependent key dispatch, and the `when` clause already being parsed | 実装済み |
| UI-108 | Tab-order focus traversal, and the fact that Tab is not bound to it | 実装済み |
...
- **Action as a type-erased, registered value** — Zed `crates/gpui/src/action.rs:117 (trait Action), :233 (ActionRegistry), :293 (insert_action), :351 (build_action)`
...
- **KeyBinding: a keystroke *sequence* with prefix matching** — Zed `crates/gpui/src/keymap/binding.rs:10 (struct), :48 (load, splits on whitespace), :89 (match_keystrokes returning Option<bool> where the bool means "pending")`
...
- **Multi-keystroke pending state and replay** — Zed `crates/gpui/src/key_dispatch.rs:116 (Replay), :121 (DispatchResult with pending / pending_has_binding / bindings / to_replay), :483 (dispatch_key), :523 (flush_dispatch), :538 (replay_prefix); crates/gpui/src/window.rs:4868-4933 (pending buffer, focus invalidation, timeout)`
...
この層の設計判断 14 件（実装済み 13）:

| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-012 | Shared native sidebar keeps panel-level visual hierarchy | 実装済み |
| UI-030 | Workspace navigation owns explicit contrast | 実装済み |
| UI-031 | Git navigation uses explicit native button states | 実装済み |
| UI-033 | Git controls preserve labels at narrow dock widths | 実装済み |
| UI-037 | Files creation actions use explicit dark-theme contrast | 実装済み |
| UI-082 | Adopt Zed One themes and a single comfortable editor line metric | 実装済み |
| UI-083 | Use Zed One terminal palettes as the native terminal source of truth | 実装済み |
| UI-091 | Use one Zed-style picker surface for Command Palette and Quick Open | 実装済み |
| UI-095 | Use Zed search glyph geometry and one find-row baseline | 実装済み |
| UI-112 | 焼き込まれた色補正 `#fafafa` → `#fcfcfc` は、AppKit と Metal の色管理差の症状 | 実装済み |
| UI-122 | 罫線の色が観測値になっていた（高さの指摘は私の測定ミス） | 実装済み |
| UI-131 | ステータスバーのアイコンが Zed より大きい | 調査のみ |
| UI-145 | Keep the native Files selection row readable while inactive | 実装済み |
| UI-153 | Clone Zed's Project Panel presentation in the native Files dock | 実装済み |
...
- **Icon source abstraction and square hit box** — Zed `crates/ui/src/components/icon.rs:117 (IconSource), :131 (Icon), :86 (square_components), :102 (square)`
- **ListItem slot layout** — Zed `crates/ui/src/components/list/list_item.rs:10 (ListItemSpacing), :26 (struct), :292 (render), :404 (disclosure at left:-1rem), :434 (EndSlotVisibility Always/OnHover/SwapOnHover)`
...
この層の設計判断 55 件（実装済み 45）:

| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-002 | Own workspace layout as application state before rendering it | 実装済み |
| UI-003 | Persist workspace composition as scalar session state | 実装済み |
| UI-004 | PaneTree is the canonical pane-geometry owner | 実装済み |
| UI-006 | Dock list selection is application state, not a text-overlay side effect | 実装済み |
| UI-007 | Files Dock opens into the focused pane | 実装済み |
| UI-008 | Tab presentation and activation are pane-local | 実装済み |
| UI-009 | Close requests resolve through the focused Pane | 実装済み |
| UI-010 | Empty editor panes expose a native welcome surface | 実装済み |
| UI-021 | Every document tab exposes its close target | 実装済み |
| UI-024 | The workspace header contains real navigation | 実装済み |
| UI-025 | Persistent workspace navigation shows its active destination | 実装済み |
| UI-027 | A left activity bar owns persistent workspace destinations | 置換 |
| UI-028 | Pane chrome exposes the current split action | 実装済み |
| UI-032 | Background file refresh never replaces the active panel | 実装済み |
| UI-035 | Git tab selection maps behavior, not enum order | 実装済み |
| UI-036 | Document tabs never paint past the editor edge | 置換 |
| UI-038 | Many document tabs retain readable labels | 実装済み |
| UI-039 | Status messages and cursor position are distinct fields | 実装済み |
| UI-041 | Persist the secondary pane item separately from the primary tab | 実装済み |
| UI-043 | Expose both supported split axes through the macOS UI | 実装済み |
| UI-045 | Bind native Save Panels to their initiating tab | 実装済み |
| UI-050 | Navigation commands follow the focused split pane | 実装済み |
| UI-055 | Bound navigation lists at the presentation boundary | 実装済み |
| UI-060 | Invert resize coordinates for the macOS right Project dock | 実装済み |
| UI-061 | Keep editor-tab actions at the tab that invoked them | 実装済み |
| UI-063 | Pinning is tab order, not a transient decoration | 実装済み |
| UI-064 | Reopen closed tabs from paths, never discarded buffers | 実装済み |
| UI-065 | Drag reordering does not implicitly change a pin | 実装済み |
| UI-067 | Constrain native editor overlays to their owning pane | 実装済み |
| UI-068 | Preserve usable workspace and pane minimum sizes | 実装済み |
| UI-070 | Let a narrowed workspace collapse native sidebar presentation | 実装済み |
| UI-071 | Do not reserve a second macOS titlebar in workspace content | 実装済み |
| UI-073 | An open workspace replaces the launch welcome surface | 実装済み |
| UI-075 | Filesの選択identityはアクティブ文書のcanonical pathで再解決する | 実装済み |
| UI-077 | ワークスペースの空エディタはWelcomeを表示しFilesを残す | 実装済み |
| UI-078 | macOS workspace presenters are single-row and pane-clipped | 実装済み |
| UI-090 | Port Zed's status-bar order and summaries | 置換 |
| UI-092 | Replace the left activity rail with Zed-style status-bar panel buttons | 置換 |
| UI-093 | Condense the status-bar dock controls to a Zed-style left cluster | 実装済み |
| UI-096 | Keep footer status semantics text-only and remove dead breadcrumb actions | 実装済み |
| UI-097 | Group the terminal toggle with the status-bar dock controls | 置換 |
| UI-113 | ステータスバーの項目は Zed の既定表示条件に従う | 実装済み |
| UI-114 | ステータスバー左側の 7 項目（未着手） | 調査のみ |
| UI-115 | 診断サマリーを Zed の判定と一致させる | 一部 |
| UI-116 | 検索ボタンの文言と設定 | 実装済み |
| UI-117 | ステータスバーの左右の振り分けは、ドックの位置で決まる | 実装済み |
| UI-118 | パネルが自分の位置を持つ構造（UI-117 の 4 番目） | 一部 |
| UI-119 | 設定が変わったときのパネルの引き取り直し（UI-118 の 5） | 実装済み |
| UI-120 | git blame をステータスバーに出す（UI-114 の 3） | 実装済み |
| UI-127 | アクティビティインジケータの表示（UI-126 の 6） | 実装済み |
| UI-128 | パンくずのシンボルは構文の色で描く | 実装済み |
| UI-130 | セッションが無いときドックを開かない | 実装済み |
| UI-132 | タイトルバーの中身が違う | 一部 |
| UI-133 | フォルダを開いた条件で出た差 | 調査のみ |
| UI-144 | Synchronize the selected tab at the session/UI composition boundary | 実装済み |
...
- **Panel size persistence and dock zoom** — Zed `crates/workspace/src/dock.rs:361 PANEL_SIZE_STATE_KEY, dock.rs:547 set_panel_zoomed, workspace.rs:4163 toggle_dock`
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-040 | Split panes own independent syntax buffers | 実装済み |
| UI-074 | macOS新規エディタはソフトラップを既定で有効にする | 置換 |
| UI-076 | 未折返し編集は主・副ペイン共通の横スクロール境界を使う | 実装済み |
| UI-079 | Make unwrapped editor scrolling Zed-compatible | 実装済み |
| UI-080 | Expose syntax selection expansion through the macOS editor | 実装済み |
| UI-099 | Preserve the fractional editor scroll phase | 実装済み |
| UI-142 | Keep split-pane viewport calculations local | 実装済み |
| UI-143 | Make multi-selection a user-visible editor feature | 実装済み |
| UI-147 | Reconstruct syntax siblings from Tree-sitter ranges | 実装済み |
| UI-148 | Keep syntax folding as an item-owned display map | 実装済み |
| UI-149 | Derive structural brackets from the syntax snapshot | 実装済み |
| UI-150 | Keep fold commands semantically separate | 実装済み |
| UI-151 | Use a continuous macOS editor scroll position | 実装済み |
| UI-152 | Keep wheel scrolling independent from cursor visibility | 実装済み |

**一部のみ — 先に片付ける**

- **DisplayPoint / DisplayRow — the coordinate space the renderer paints in** — Zed `crates/editor/src/display_map.rs:2496 (DisplayPoint(BlockPoint)), :2526 (DisplayRow), :2646 (DisplayPointConverter)`
...
- **SelectionsCollection: disjoint anchored selections plus a pending one** — Zed `crates/editor/src/selections_collection.rs:26 (SelectionsCollection), :20 (PendingSelection), :547 change_with, :978 move_with, :1027 move_heads_with`
- **Anchor-based scroll position** — Zed `crates/editor/src/scroll.rs:38 (ScrollAnchor {anchor, offset}), :388 ScrollManager::scroll_position, :51 ScrollAnchor::scroll_position`
- **Autoscroll strategies** — Zed `crates/editor/src/scroll/autoscroll.rs:16 (enum Autoscroll), :100 (AutoscrollStrategy), :127 autoscroll_vertically, :345 autoscroll_horizontally; request queued on ScrollManager at scroll.rs:214`
...
- **Scrollbar geometry and thumb state** — Zed `crates/editor/src/scroll.rs:181 (ScrollbarThumbState), :189 (ActiveScrollbarState), :522 show_scrollbars, SCROLLBAR_SHOW_INTERVAL at :31`
- **Edit transactions grouped with selection history** — Zed `crates/editor/src/editor.rs:8286 transact, :8299 start_transaction_at, :8321 end_transaction_at, :1394 SelectionHistory, :1297 SelectionHistoryMode`
- **Highlight layering on top of the transform chain** — Zed `crates/editor/src/display_map.rs:213 (text_highlights, inlay_highlights, semantic_token_highlights fields), :161 (HighlightKey), :334 (HighlightStyleInterner), :1408 (HighlightedChunk); custom_highlights.rs:30`
...
### エディタ: 描画（一部 13 / 無 8 / 不明 1）

この層の設計判断 10 件（実装済み 9）:

| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-044 | Render Git hunks from each visible pane's document | 実装済み |
| UI-081 | Make editor and project-panel chrome share hard Zed-style edges | 実装済み |
| UI-087 | Clip every editor overlay in the same local text viewport | 実装済み |
| UI-089 | Synchronize editor horizontal scrolling before retained paint | 実装済み |
| UI-094 | Make editor chrome container-owned and overlay-safe | 実装済み |
| UI-098 | Match Zed's singleton editor gutter geometry | 実装済み |
| UI-121 | 行内 blame（カーソル行の行末に出す） | 実装済み |
| UI-124 | Markdown 見出しの色が観測値のせいでテーマから離れている | 実装済み |
| UI-134 | ガターの変更バー（UI-133 の 4） | 調査のみ |
| UI-155 | Snap editor rows and wrap width to Zed's pixel contract | 実装済み |

**一部のみ — 先に片付ける**

- **GutterDimensions — the gutter geometry contract** — Zed `crates/editor/src/editor.rs:1246 (struct), crates/editor/src/editor.rs:11546 (EditorSnapshot::gutter_dimensions), crates/editor/src/fold.rs:5 (fold_area_width)`
...
- **LineWithInvisibles::from_chunks — the line shaping pipeline** — Zed `crates/editor/src/element.rs:7013 (struct), crates/editor/src/element.rs:7045 (from_chunks), crates/editor/src/element.rs:7021 (enum LineFragment)`
...
- **The prepaint→paint split and EditorLayout** — Zed `crates/editor/src/element.rs:7954 (prepaint), :9598 (struct EditorLayout), :9431 (paint)`
- **paint_background — active line and highlighted rows** — Zed `crates/editor/src/element.rs:4890`
- **items.rs tab_content — the tab label element** — Zed `crates/editor/src/items.rs:751; MAX_TAB_TITLE_LEN=24 at items.rs:66; entry_git_aware_label_color at items.rs:2200; entry_label_color at items.rs:2172`
- **breadcrumbs — segments from the editor, rendering from the element** — Zed `crates/editor/src/items.rs:1069 (breadcrumb_location), :1078 (breadcrumbs), :1089 (breadcrumb_prefix); crates/editor/src/element.rs:6726 (render_breadcrumb_text), :6883 (apply_dirty_filename_style)`
- **Editor blame lifecycle and inline-blame gating** — Zed `crates/editor/src/git.rs:503 (git_blame_inline_enabled), :541 (show_git_blame_gutter), :556 (toggle_git_blame), :571 (toggle_git_blame_inline), :589 (hide_blame_popover), :949 (show_blame_hover_popover), :2131 (start_git_blame)`
...
- **PositionMap::point_for_position — pixels to a clipped DisplayPoint** — Zed `crates/editor/src/element.rs:10075 (struct PositionMap), crates/editor/src/element.rs:10142`


### プロジェクト層（一部 9 / 無 4 / 不明 1）

この層の設計判断 9 件（実装済み 8）:

| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-011 | Git panel resolves workspace context without an open document | 実装済み |
| UI-029 | Git panel resolves its branch asynchronously | 実装済み |
| UI-042 | Keep LSP document lifetimes and diagnostic buffers pane-local | 実装済み |
| UI-046 | Keep Git history bound to its source repository | 実装済み |
| UI-056 | Scope folder search through the workspace path resolver | 実装済み |
| UI-057 | Never signal a shared process group from a UI cancellation action | 実装済み |
| UI-126 | LSP の `$/progress`（アクティビティインジケータの前提） | 実装済み |
| UI-129 | 長時間の git ジョブ（アクティビティインジケータの源 4） | 却下 |
| UI-146 | Keep Outline useful before LSP symbols arrive | 実装済み |

**一部のみ — 先に片付ける**

- **Worktree snapshot + background scanner with scan ids** — Zed `crates/worktree/src/worktree.rs:176 (Snapshot), :249 (LocalSnapshot), :270 (BackgroundScannerState), :410 (enum ScanState), :4267 (BackgroundScanner), :4288 (BackgroundScannerPhase)`
- **Repository as an entity with a snapshot + a serialized job queue** — Zed `crates/project/src/git_store.rs:476 (struct Repository), :401 (RepositorySnapshot), :607 (GitJob), :614 (enum GitJobKey), :498 (Deref<Target=RepositorySnapshot>)`
...
- **Store composition + event re-emission (Project as hub)** — Zed `crates/project/src/project.rs:214 (struct Project), :335 (enum Event), :1201-1329 (cx.subscribe wiring), :3581 on_buffer_store_event, :3638 on_lsp_store_event, :3866 on_worktree_store_event`
...
- **Per-buffer diff bases (head text / index text) and diff recalculation** — Zed `crates/project/src/git_store.rs:120 (BufferGitState: head_text, index_text, head_text_buffer, index_text_buffer, head_changed, index_changed), :184 (DiffBasesChange), :195 (DiffKind), :4671 (recalculate_diffs)`
- **Language server lifecycle: Starting/Running state and the seed key that decides identity** — Zed `crates/project/src/lsp_store.rs:14783 (enum LanguageServerState Starting{startup task, pending_workspace_folders} / Running{adapter, server, ...}), :267 (LanguageServerSeed: worktree_id + name + toolchain + settings), :251 (UnifiedLanguageServer with project_roots), :350 get_or_insert_language_server`
...
### 機能 crate（一部 13 / 無 6 / 不明 1）

この層の設計判断 37 件（実装済み 35）:

| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-001 | UI を機能の入口として扱う | 実装済み |
| UI-013 | Files panel owns contextual workspace actions | 実装済み |
| UI-014 | Git navigation is visible in the sidebar | 実装済み |
| UI-015 | Files creation is visible at the point of use | 実装済み |
| UI-016 | Git status rows own safe stage actions | 実装済み |
| UI-017 | Git history exposes commit-level actions | 実装済み |
| UI-018 | Command Palette presents discoverable primary actions | 実装済み |
| UI-019 | Terminal sessions are explicit panel controls | 実装済み |
| UI-020 | Commit details use an identified, dismissible inspector | 実装済み |
| UI-022 | An empty Files panel starts a workspace | 実装済み |
| UI-023 | Git has an explicit no-repository state | 実装済み |
| UI-026 | Git Changes exposes a native commit entry point | 実装済み |
| UI-034 | Git status has one primary presentation | 実装済み |
| UI-047 | Make changed-file diffs visible from the Git panel | 実装済み |
| UI-048 | Enter file history from the file tree | 実装済み |
| UI-049 | Copy a file-tree entry path without opening it | 実装済み |
| UI-051 | Make Git change actions follow their rendered section | 実装済み |
| UI-052 | Keep Quick Open out of the editor document surface | 実装済み |
| UI-053 | Give workspace search its own navigation panel | 実装済み |
| UI-054 | Open an integrated terminal from a project entry | 実装済み |
| UI-059 | Keep workspace-search controls inside the Search panel | 実装済み |
| UI-062 | Branch context actions are non-destructive by default | 実装済み |
| UI-066 | Edit supported settings without blocking the workspace | 実装済み |
| UI-072 | External file changes must not block the editor window | 実装済み |
| UI-084 | Clone Zed's Outline and diagnostic presentation | 実装済み |
| UI-085 | Keep Files panel creation and trash actions at the selection boundary | 実装済み |
| UI-086 | Make implemented editor services discoverable from Command Palette | 実装済み |
| UI-088 | Route macOS View menu actions through the existing command boundary | 実装済み |
| UI-135 | Model Git History as explicit loading, error, and loaded states | 実装済み |
| UI-136 | Make Files-panel tree navigation continuously reachable | 実装済み |
| UI-137 | Keep document search and navigation inside the editor surface | 実装済み |
| UI-138 | Present the command palette as editor chrome, not an alert | 置換 |
| UI-139 | Start workspace search without a blocking prompt | 実装済み |
| UI-140 | Edit a Git commit message in the Changes surface | 置換 |
| UI-141 | Keep Quick Open inside the editor workflow | 実装済み |
| UI-154 | Port Zed's option-aware search bars and project filters | 実装済み |
| UI-156 | Make Git Changes rows native checkbox controls with a pinned commit footer | 実装済み |
...
- **Debugger panel / DAP client** — Zed `crates/debugger_ui/src/debugger_panel.rs:1541 impl Panel; crates/dap (2800 lines), initialised at crates/zed/src/main.rs:593`
...
- **Unified sidebar (newer container)** — Zed `crates/sidebar/src/sidebar.rs (8441 lines, 3 files, 23736 total)`
...
| 番号 | 表題 | 状態 |
| --- | --- | --- |
| UI-058 | Keep GUI acceptance independent of terminal-session hangups | 実装済み |
