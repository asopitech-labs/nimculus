...
- [x] **Sprite atlas with shelf packing and keyed tiles** — Zed `crates/gpui_macos/src/metal_atlas.rs:13 (MetalAtlas), :40 (get_or_insert_with), :96 (allocate), :121 (push_texture), :62/:250 (remove + refcount)`
...
- [x] **Hitbox list + topmost hit testing with blocking behaviors** — Zed `crates/gpui/src/window.rs:799 struct Hitbox, :724 enum HitboxBehavior, :4820 insert_hitbox, :1059 Frame::hit_test, :2919 (per-frame cache)`
