...
Zed のコード構成を層ごとに整理し、各メカニズムを Nim でどう再現するかを決めるための文書。
**個別の差分を追いかける前にここを見る。**

2026-08-10 に 10 のエージェントで Zed と Nimculus を並行して読み、突き合わせて作った。
行番号は実測。読んでいないものは「不明」と記す。**エージェントの記述は切り捨てずに全文を載せている。**

関連文書: 移植タスクと残作業は [`ZED_PORT_TASKS.md`](ZED_PORT_TASKS.md)、
個々の判断は [`../DESIGN_DECISIONS.md`](../DESIGN_DECISIONS.md)。
...
**34 倍。全部は移植できない。** 何を移植し何を移植しないかを決めるための文書がこれ。

## メカニズムの移植状況

| 層 | 計 | 移植済み | 一部のみ | 無い | 不明 |
| --- | ---: | ---: | ---: | ---: | ---: |
| [プラットフォーム層（OS 境界）](#プラットフォーム層（OS 境界）) | 26 | 3 | 10 | 13 | 0 |
| [フレームワーク: 中核](#フレームワーク: 中核) | 13 | 0 | 7 | 6 | 0 |
| [フレームワーク: テキストと描画](#フレームワーク: テキストと描画) | 17 | 3 | 7 | 7 | 0 |
| [フレームワーク: 入力とアクション](#フレームワーク: 入力とアクション) | 17 | 2 | 8 | 7 | 0 |
| [デザインシステム層](#デザインシステム層) | 16 | 0 | 9 | 7 | 0 |
| [ワークスペース層](#ワークスペース層) | 19 | 3 | 12 | 4 | 0 |
| [エディタ: モデル](#エディタ: モデル) | 19 | 1 | 11 | 7 | 0 |
| [エディタ: レイアウトと描画](#エディタ: レイアウトと描画) | 24 | 2 | 13 | 8 | 1 |
| [プロジェクト層（サービス）](#プロジェクト層（サービス）) | 16 | 2 | 9 | 4 | 1 |
| [機能 crate 群](#機能 crate 群) | 23 | 3 | 13 | 6 | 1 |
| **合計** | **190** | **19** | **99** | **69** | **3** |

**過半数（99 件、52%）が「一部のみ」。**
移植済みは 19 件しかないのに、着手した痕跡があるものが 99 件ある。
「一部のみ」は動いているように見えて条件を変えると壊れる。
2026-08-09〜10 に見つけた不具合はほぼ全部この 99 件の中にあった。
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/gpui_macos/src/window.rs` | 3228 | MacWindow: runtime synthesis of the NSView and NSWindow/NSPanel subclasses (:131-475), MacWindowState (:490-748), window open (:755-1103), the PlatformWindow impl (:1205-1897), and every ObjC callback trampoline (key, mouse, IME, drag, resize, fullscreen). |
| `crates/gpui_macos/src/metal_renderer.rs` | 1799 | MetalRenderer: device/CAMetalLayer setup (:150-176), instance-buffer pool (:56-109), the eight render pipeline states (:111-140), drawable-size and path intermediate textures (:381-433), and the per-primitive-kind draw passes (:446-1568). |
| `crates/gpui_macos/src/platform.rs` | 1514 | MacPlatform: NSApplication/app-delegate class synthesis (:70-163), the whole Platform trait impl (:475-1193), menu-bar construction (:241-460), open/save panels (:744, :794), keychain (:1089-1192), and the Carbon/TIS + Security extern blocks (:1461-1514). |
...
| `crates/gpui_macos/src/text_system.rs` | 904 | MacTextSystem: CoreText font loading/matching (:276-419), CoreGraphics glyph rasterization including subpixel positioning and dilation (:421-530), and CTLine-based single-line shaping (:532-628). |
...
| `crates/gpui_macos/src/display_link.rs` | 461 | Frame pacing: an immortal per-display CVDisplayLink registry (:65-226) plus WindowFrameSource, a per-window GCD data-add source that coalesces vsync ticks onto the main queue (:231-283). |
...
| `crates/gpui_macos/src/display.rs` | 169 | MacDisplay: CGDirectDisplayID enumeration, UUID identity, and bounds/visible_bounds in GPUI's top-left coordinate space (:16-151). |
| `crates/gpui_macos/src/gpui_macos.rs` | 136 | Crate root: declares the submodules, and holds the shared ObjC glue (NSRange repr at :84, ns_string at :134, NSStringExt::to_str at :70). |
| `crates/gpui_macos/src/window_appearance.rs` | 35 | Maps NSAppearance to gpui WindowAppearance (light/dark/vibrant). |

### Nimculus の対応物

| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m` | 17976 | Everything: NSApp bootstrap and delegate, the NSWindow, NimculusMetalView with Metal layer + CADisplayLink + NSTextInputClient, the glyph atlas and Metal draw pass, CoreText shaping, clipboard, accessibility, menus, sheets — and also ~40 AppKit-drawn application widgets (tab bar, sidebar, terminal overlay, footer, git panels, command palette, settings) plus ~60 in-file nimculus_platform_validate_* self-tests. |
| `/Users/yoshinori/work/nimculus/src/nimnui/text.nim` | 467 | The framework side of shaping: LineLayoutCache (:119), layoutLineByHash (:332), layoutWrappedLineByHash (:367) — the analogue of gpui's line layout cache, sitting above nimculus_platform_layout_line. |
...
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/platform.h` | 290 | The declared macOS boundary. Roughly 200 entry points, of which only a handful (run, layout_line, clipboard, get_metrics, the four dispatch calls via contracts.h) are platform-level; the rest are editor-domain (set_editor_git_hunks, set_editor_sidebar_line_items, show_git_status_context, set_terminal_runs). |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/contracts.h` | 240 | The genuinely portable contracts: PlatformMetrics, InputEvent, the text-shaping structs (:72-90) with an explicit note that the framework owns rows/wrapping/caching, the accessibility node (:189-209), the paint command/region structs, and the dispatcher ABI (:220-238). |
...
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/text_platform.h` | 19 | A second, older text boundary: font availability, font enumeration, and whole-string measurement — parallel to and separate from the layout_line contract in contracts.h. |
...
#### The Platform trait as the OS boundary — 無い

Zed: `crates/gpui_macos/src/platform.rs:475 (impl Platform for MacPlatform)` / Nimculus: `src/nimnui/platform/contracts.h (240 lines, dispatcher only) vs src/nimnui/platform/macos/platform.h (290 lines, ~200 app-specific entry points)`
...
#### Runtime NSView/NSWindow subclass synthesis with a fixed selector set — 一部のみ

Zed: `crates/gpui_macos/src/window.rs:131-303 (VIEW_CLASS), :365-474 (build_window_class)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:4110 (@interface NimculusMetalView : NSView <NSTextInputClient>), :9515-11034 implementation; window delegate methods at :11060-11095`
...
#### Metal renderer: pipeline states per primitive kind and a pooled instance buffer — 一部のみ

Zed: `crates/gpui_macos/src/metal_renderer.rs:111-140 (MetalRenderer fields), :56-109 (InstanceBufferPool), :446 (draw), :1047-1568 (draw_shadows/quads/paths/underlines/mono+poly sprites/surfaces)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:1285-1289 (g_pipeline, g_text_pipeline, g_glyph_pipeline, g_polychrome_glyph_pipeline, g_image_pipeline), drawFrame :10441-10735`
...
#### Sprite atlas with shelf packing and keyed tiles — 一部のみ

Zed: `crates/gpui_macos/src/metal_atlas.rs:13 (MetalAtlas), :40 (get_or_insert_with), :96 (allocate), :121 (push_texture), :62/:250 (remove + refcount)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:1294-1308 (g_glyph_atlas_texture, g_glyph_atlas_next_x/next_y/row_height, hit/miss/eviction counters), :1376 (g_glyph_atlas_cache hash table), :3429-3462 (rebuild on scale change)`
...
#### Single-line text shaping (CTLine) as the platform's only text job — 移植済み

Zed: `crates/gpui_macos/src/text_system.rs:532 (layout_line)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:3279 (nimculus_platform_layout_line), contract in src/nimnui/platform/contracts.h:72-90, Nim-side cache at src/nimnui/text.nim:119 (LineLayoutCache), :332 (layoutLineByHash), :367 (layoutWrappedLineByHash)`
...
#### Glyph rasterization with subpixel variants and dilation — 無い

Zed: `crates/gpui_macos/src/text_system.rs:421 (raster_bounds, dilated by 1px), :436 (rasterize_glyph)` / Nimculus: `not exposed in src/nimnui/platform/contracts.h; rasterization happens privately inside macos_platform.m's atlas path`
...
#### Font fallback cascade and OpenType features — 無い

Zed: `crates/gpui_macos/src/open_type.rs:34 (apply_features_and_fallbacks), :102 (generate_fallback_array), :155 (append_system_fallbacks)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:2859-2877 creates a CTFont with a descriptor; contracts.h:92 declares NimculusEditorFontFeature but platform.h exposes no function taking it`
...
#### Task dispatcher over GCD — 移植済み

Zed: `crates/gpui_macos/src/dispatcher.rs:31-79 (PlatformDispatcher impl), :166 (trampoline)` / Nimculus: `src/nimnui/platform/contracts.h:220-238, src/nimnui/platform/macos/macos_platform.m:69-91, src/nimnui/platform/dispatcher.nim:35-46, src/nimnui/platform/macos/platform.nim:18-20`
...
#### Keyboard layout identity and per-layout key-equivalent remapping — 無い

Zed: `crates/gpui_macos/src/keyboard.rs:13 (MacKeyboardLayout), :18 (MacKeyboardMapper), :30-50 (map_key_equivalent), plus ~1400 lines of per-layout character tables` / Nimculus: `no TIS call in src/nimnui/platform/macos/macos_platform.m`
...
#### NSEvent to portable input event translation — 一部のみ

Zed: `crates/gpui_macos/src/events.rs:258-286 (scroll phase and precise vs line deltas), :288-300 (button numbers incl. Navigate back/forward), crates/gpui_macos/src/window.rs:319 (convert_mouse_position, y-flip)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:4058 (logInput), contract struct at src/nimnui/platform/contracts.h:18-35`
...
#### Pasteboard read/write with typed content — 一部のみ

Zed: `crates/gpui_macos/src/pasteboard.rs:22 (Pasteboard), :50 (read), :92 (read_image), :165 (write), :264-333 (UTType mapping for png/jpeg/gif/webp/bmp/svg/ico/tiff/pnm)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:17921 (clipboardTextFromPasteboard), :17933 (nimculus_clipboard_set), :17945/:17952 (read back)`
...
#### Display enumeration and coordinate space — 無い

Zed: `crates/gpui_macos/src/display.rs:16 (MacDisplay), :28 (primary), :48 (all), :79 (uuid), :108 (bounds), :121 (visible_bounds)` / Nimculus: `no display enumeration in src/nimnui/platform/macos/macos_platform.m; the window is created and [window center]'d at :12109-12159`
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/gpui/src/window.rs` | 6551 | Owns one window: the two-Frame (rendered/next) retained draw state, the WindowInvalidator/DrawPhase state machine, the request_layout->prepaint->paint->focus->present pipeline, hitboxes, element state, content masks and element offsets. |
...
| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | Everything App and Window own in Zed, plus workspace/editor/project. setupDemoUi (main.nim:334-655) is the entire frame - rebuild tree, layout, compose paint list, publish to platform - invoked by hand from 42 call sites. |
| `/Users/yoshinori/work/nimculus/src/nimnui/ui_tree.nim` | 293 | Retained node tree: ids+generations (approx EntityId handles), per-node dirty bits (approx WindowInvalidator but per node), hitTest (approx Frame::hit_test), focus/tab-stop and a11y fields, KeyContext dispatch path (approx dispatch_tree focus_path). |
| `/Users/yoshinori/work/nimculus/src/nimnui/layout.nim` | 289 | The whole layout phase: flexbox main/cross sizing, absolute positioning, clip propagation. Stands in for request_layout + compute_layout + Taffy, as one recursive pass with no LayoutId indirection and no measured layout. |
...
#### Entity handle + type-erased entity store (lease discipline) — 無い

Zed: `crates/gpui/src/app/entity_map.rs:414 (Entity<T>), :114 reserve, :120 insert, :134 lease, :151 end_lease, :156 read` / Nimculus: `src/nimculus/main.nim - state is ~120 module-level vars (main.nim:142 demoTree, :256 imeState, :257 editorSession, :911 activeWorkspace)`
...
#### Context<'a,T> - entity-scoped view of App — 無い
...
#### Typed pixel units and the scale-factor ladder — 一部のみ
...
#### Bounds / Point / Size / Edges / Corners generic over unit — 一部のみ

Zed: `crates/gpui/src/geometry.rs:85 Point<T>, :396 Size<T>, :723 Bounds<T>, :1750 Edges<T>, :2258 Corners<T>, :43 trait Along, :25 enum Axis, :1694 Bounds<Pixels>::scale, :1707 to_device_pixels` / Nimculus: `src/nimnui/geometry.nim:4 Point, :7 Size, :10 Rect, :14 EdgeInsets`
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/gpui/src/window.rs` | 6551 | The paint_* boundary that converts logical Pixels into ScaledPixels scene primitives: paint_quad, paint_path, paint_underline, paint_strikethrough, paint_glyph, paint_emoji, paint_svg, paint_image, paint_surface, paint_layer. |
| `crates/gpui/src/text_system/line_wrapper.rs` | 1577 | The pooled LineWrapper used for soft-wrap boundary search independent of a shaped layout; obtained via TextSystem::line_wrapper (text_system.rs:307). I read only its declaration surface, not its body. |
...
| `crates/gpui/src/text_system.rs` | 1206 | Owns the process-wide TextSystem: font id interning, font metrics cache, glyph raster-bounds cache, the line-wrapper pool, and the per-window WindowTextSystem that fronts the line layout cache and the shape_line/shape_text entry points. |
| `crates/gpui/src/text_system/line_layout.rs` | 1078 | Defines LineLayout/ShapedRun/ShapedGlyph/WrappedLineLayout, the index<->x geometry queries, wrap-boundary computation, and the two-frame LineLayoutCache (including the hash-keyed, allocation-free variant). |
...
| `crates/gpui/src/text_system/line.rs` | 1015 | ShapedLine/WrappedLine: pairs a cached LineLayout with DecorationRuns and turns them into paint calls (glyph/emoji sprites, underline/strikethrough runs, per-run background quads), plus split_at and precomputed GlyphRasterData. |
| `crates/gpui/src/scene.rs` | 915 | The Scene: per-kind primitive vectors, layer stack + BoundsTree draw order assignment, insert/replay/finish (sort by order), and the BatchIterator that merges the eight sorted streams into draw batches. |
...
| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m` | 17976 | The whole rendering half of this layer: RenderGlyphParams-equivalent atlas key and 4x1 subpixel quantization, mono/polychrome sprite structs and their Metal pipelines, per-row glyph walk and baseline computation, ch_width/ch_advance/em_width metric extraction for gutter geometry, glyph atlas shelf allocation and eviction. Contains no Quad/Underline/Shadow/Path primitive, no draw order, no batching, and no Hsla. |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | Not read for this layer; noted only because it is where the application-level style and colour decisions that Zed puts in TextStyle/theme would have to live today. |
| `/Users/yoshinori/work/nimculus/src/nimnui/text.nim` | 467 | LineLayout/ShapedRun/ShapedGlyph/WrappedLineLayout types, the two-frame hash-keyed LineLayoutCache with steal-from-previous and finishFrame swap, computeWrapBoundaries, the FFI declaration of the macOS one-line shaper, plus an unused-looking GlyphKey/GlyphAtlas pair that duplicates the raster key already implemented in C. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_text_layout.nim` | 230 | The app-side half of shape_text: font-run construction from syntax decorations (the analogue of the FontRun coalescing in text_system.rs:556-567), wrapped-row expansion, and the index->x query. |
| `/Users/yoshinori/work/nimculus/src/nimnui/render.nim` | 170 | The paint-command list that stands in for Zed's Scene: a single PaintCommand object with a semantic kind enum, dirty-rect merging, a clip stack and a transform stack. Roughly Zed's PaintOperation/StartLayer/EndLayer, but with no per-kind streams and no order. |

### メカニズム

#### Font identity and FontId interning — 一部のみ

Zed: `crates/gpui/src/text_system.rs:1051 (Font), :107 (font_id), :148 (resolve_font)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/text.nim:42 (FontRun.fontId: uint32); /Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m:1355-1368 (font_id in the raster key)`
...
#### Font metrics cache and the em_width / em_advance / ch_width / ch_advance distinction — 一部のみ

Zed: `crates/gpui/src/text_system.rs:226-249, :292 (read_metrics), :1104 (FontMetrics), :1137-1177` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m:2164-2165, :2187-2189 (editorGlyphAdvance), :2212-2215, :3068 (emWidth via typographic width of 'm')`
...
#### Two-frame line layout cache with hash keys — 移植済み

Zed: `crates/gpui/src/text_system/line_layout.rs:392 (LineLayoutCache), :398 (FrameCache), :497 (finish_frame), :577 (layout_line), :638/:694 (by-hash variants)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/text.nim:119 (LineLayoutCache), :115 (FrameCache), :332 (layoutLineByHash), :367 (layoutWrappedLineByHash), :409 (finishFrame)`
...
#### Line geometry queries (index_for_x, x_for_index, position_for_index) — 一部のみ

Zed: `crates/gpui/src/text_system/line_layout.rs:58, :75, :105, :117, :283-362` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/editor_text_layout.nim:97 (xAt), :92 (glyphByteAt)`
...
#### Wrap boundary computation on the already-shaped glyph stream — 移植済み

Zed: `crates/gpui/src/text_system/line_layout.rs:128 (compute_wrap_boundaries), :212 (WrappedLineLayout), :225 (WrapBoundary)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/text.nim:272 (computeWrapBoundaries), :70 (WrapBoundary), :73 (WrappedLineLayout)`
...
#### paint_line: turning a layout plus decorations into primitives — 一部のみ

Zed: `crates/gpui/src/text_system/line.rs:334-578, background variant at :580` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m:2904-2919 (editorTextBaselineFromMetrics), :3736 (appendEditorGlyphSprite), :3795-3830 (per-row glyph walk)`
...
#### Draw order, the layer stack, and the BoundsTree — 無い
...
#### Batching: merging sorted streams into draw calls — 無い
...
#### Hsla and alpha derivation — 無い

Zed: `crates/gpui/src/color.rs:334 (Hsla), :424 (hsla), :525 (to_rgb), :580 (blend), :607 (fade_out), :637 (opacity), :667 (alpha), :677 (From<Rgba>)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimnui/text.nim:51 (TextRun.color: array[4, float32]); /Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m:1384-1389 (NimculusGlyphColor rgba)`
...
#### TextStyle and its refinement/highlight composition — 無い
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/gpui/src/window.rs` | 6551 | Runtime side of the layer: FocusId/FocusHandle, DispatchPhase, and the capture/bubble driving of key listeners, modifier listeners and action listeners (read only around lines 88, 267, 383-470, 4805-5200). |
...
| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | All actual keymap content and dispatch wiring: the global registry (:143), the native shortcut entry point (:663), ~30 hardcoded default bindings (:681-802) and the settings-driven rebind pass (:804). In Zed this content lives in assets/keymaps + the workspace/editor crates, not in the framework. |
| `/Users/yoshinori/work/nimculus/src/nimnui/ui_tree.nim` | 293 | Focus ownership, generation-checked handles, disabled-path rules, per-node KeyContext and contextStack. Covers the FocusHandle/FocusId parts of window.rs and the context-stack part of key_dispatch.rs. |
| `/Users/yoshinori/work/nimculus/src/nimnui/controls.nim` | 263 | Nothing from this layer structurally — it is overlay/menu widget state. Relevant only because handleKey (:184) and handlePointerDown (:197) hand-roll the activation routing that Zed's ClickEvent and action dispatch would provide. |
...
#### Action as a type-erased, registered value — 無い

Zed: `crates/gpui/src/action.rs:117 (trait Action), :233 (ActionRegistry), :293 (insert_action), :351 (build_action)` / Nimculus: `src/nimnui/commands.nim:14 (`Command` = name + shortcut + whenClause + closure)`
...
#### NoAction / Unbind disable markers — 無い
...
#### KeyContext: the per-node context entry list — 一部のみ

Zed: `crates/gpui/src/keymap/context.rs:10 (KeyContext), :14 (ContextEntry), :65 (parse), :117 (add), :126 (set), :137 (contains), :142 (get)` / Nimculus: `src/nimnui/context.nim:9 (KeyContext), :4 (ContextEntry), :30/:33 (constructors), :41 (contains), :44 (value); attached to nodes at src/nimnui/ui_tree.nim:42 and set at :275`
...
#### Context predicate language and depth_of — 移植済み

Zed: `crates/gpui/src/keymap/context.rs:172 (enum), :249 (parse), :350 (parse_expr, precedence table :494-498), :277 (eval_inner), :260 (depth_of), :328 (is_superset)` / Nimculus: `src/nimnui/context.nim:16 (ref-object AST), :95 (parseExpression, same precedences 1/2/3/4/5), :156 (evalInner), :187 (depthOf), :194 (eval)`
...
#### KeyBinding: a keystroke *sequence* with prefix matching — 無い

Zed: `crates/gpui/src/keymap/binding.rs:10 (struct), :48 (load, splits on whitespace), :89 (match_keystrokes returning Option<bool> where the bool means "pending")` / Nimculus: `src/nimnui/commands.nim:11 (`Shortcut` = single keyCode + `set[Modifier]`), :147 (shortcutFromKeyBinding parses only `cmd+shift+p`)`
...
#### Keymap precedence resolution (bindings_for_input) — 一部のみ

Zed: `crates/gpui/src/keymap.rs:164-242, sort at :187, binding_enabled at :245` / Nimculus: `src/nimnui/commands.nim:48 (resolve), :63 (tryResolve), :42 (matchingDepth)`
...
#### Binding source metadata (user > vim > base > default) — 無い

Zed: `crates/gpui/src/keymap/binding.rs:143 (KeyBindingMetaIndex), crates/gpui/src/keymap.rs:199 (NoAction only breaks for user-sourced bindings), keymap.rs:831 (test documenting User 0 > Vim 1 > Base 2 > Default 3)` / Nimculus: `src/nimculus/main.nim:804 (applySettingsKeymap rebuilds the whole registry from defaults, then patches shortcuts by command name)`
...
#### Reverse lookup: bindings_for_action, with shadow filtering — 無い
...
#### The DispatchTree: per-frame node tree of contexts, focus ids and listeners — 一部のみ

Zed: `crates/gpui/src/key_dispatch.rs:71 (DispatchTree), :83 (DispatchNode), :166 (push_node), :215 (set_key_context), :220 (set_focus_id), :226 (set_view_id), :323/:327/:333 (on_key_event / on_modifiers_changed / on_action)` / Nimculus: `src/nimnui/ui_tree.nim:44 (UiTree), :19 (UiNode carries context, focusable, tabIndex, tabStop, bounds, a11y all on one object), :279 (contextStack); src/nimnui/events.nim:54 (ancestorPath)`
...
#### dispatch_path / focus_path / focus_contains — 一部のみ

Zed: `crates/gpui/src/key_dispatch.rs:563 (dispatch_path, root-to-focused), :574 (focus_path), :346 (focus_contains)` / Nimculus: `src/nimnui/ui_tree.nim:279 (contextStack builds the path inline and only returns contexts), src/nimnui/events.nim:54 (ancestorPath, target-to-root)`
...
#### Multi-keystroke pending state and replay — 無い
...
#### Tab stop ordering by (group path, tab index, insertion order) — 移植済み

Zed: `crates/gpui/src/tab_stop.rs:11 (TabStopMap), :36 (TabStopPath), :40 (TabStopNode), :52 (Ord: path then insertion index), :78 (insert appends the handle's tab_index to the current group path), :92/:98 (begin_group/end_group), :111 (next, wraps to first, skips non-tab_stop nodes), :148 (prev), :185 (replay)` / Nimculus: `src/nimnui/commands.nim:162 (TabStopEntry), :169 (tabPath, walks the parent chain), :180 (compareTabPaths), :187 (tabStopOrder, sorts by path then insertion index), :198 (focusByTabOrder), :226/:228 (focusNext/focusPrev)`
...
#### available_actions / is_action_available for the command palette — 無い

Zed: `crates/gpui/src/key_dispatch.rs:363 (available_actions, walks the dispatch path collecting listener action types and building a default instance of each), :382 (is_action_available)` / Nimculus: `src/nimculus/main.nim:681-802 (setupShortcutRegistry registers a flat global list; palette entries come from that list, not from what is focused)`
...
| `crates/ui/src/components/button/button_like.rs` | 981 | The core button: ButtonStyle × elevation × interaction state -> concrete ButtonLikeStyles, ButtonSize height table, and the render that wires aria, focus, hover, active and rounding. |
...
| `crates/ui/src/components/tooltip.rs` | 291 | Tooltip view (title + meta + keybinding) and tooltip_container(), which is the elevation_2 shell every tooltip shares. |
| `crates/ui/src/styles/color.rs` | 242 | The semantic `Color` enum and its single resolution point into the active theme's colors()/status() tables. |
| `crates/ui/src/components/divider.rs` | 239 | Divider: horizontal/vertical, solid (bg fill) vs dashed (canvas PathBuilder with dash array), DividerColor tokens. |
| `crates/ui/src/traits/styled_ext.rs` | 140 | h_flex/v_flex and elevation_1/2/3 — the shorthand that turns an elevation index into bg+radius+border+shadow. |
| `crates/ui/src/styles/elevation.rs` | 118 | `ElevationIndex` — the z-layer enum that decides shadow stacks and which background token a surface uses. |
| `crates/ui/src/traits/toggleable.rs` | 69 | `Toggleable` and the tri-state `ToggleState` (Unselected/Indeterminate/Selected) with inverse() and from_any_and_all(). |
| `crates/ui/src/styles/spacing.rs` | 55 | Declares the DynamicSpacing scale as 14 (compact, default, comfortable) pixel triples fed to a derive macro. |
| `crates/ui/src/styles/units.rs` | 27 | rems_from_px / vw / vh — the px-to-rem bridge every size table goes through. |
| `crates/ui/src/ui.rs` | 20 | Crate root; re-exports components, prelude, styles, traits as one flat namespace. |
| `crates/ui/src/traits/clickable.rs` | 9 | `Clickable`: on_click + cursor_style, implemented by every button-shaped component. |
| `crates/ui/src/traits/disableable.rs` | 6 | `Disableable`: the single `disabled(bool)` builder method. |

### Nimculus の対応物

| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m` | 17976 | Where the real component library lives, as AppKit classes used directly: NimculusChromeButton (:950) is the only reusable styled control; everything else is per-feature NSButton/NSTextField/NSMenu instances (~40 declared NSButton properties across overlay classes at :4363-4553). Sizing constants at :438-473, colour resolution at :842-1099, the shortcut display table at :4685. |
| `/Users/yoshinori/work/nimculus/src/nimculus/settings.nim` | 921 | The theme token table (ThemeColors, :31, ~48 fields) and the built-in dark/light palettes (:523-578). This is Zed's theme crate's job, not the ui crate's — but it is the data the missing Color enum would resolve against. |
| `/Users/yoshinori/work/nimculus/src/nimnui/ui_tree.nim` | 293 | Interaction-state substrate: independent hovered/active/focused/disabled flags with Zed-matching precedence (:88-98), generational node handles (:12, :185), focus invalidation on disable (:227), and clip-aware hit testing (:143). This is the layer Zed's per-component hover()/active()/focus_visible() style refinements would sit on top of — but nothing sits on it yet. |
| `/Users/yoshinori/work/nimculus/src/nimnui/controls.nim` | 263 | The only file resembling a component layer. Covers menu/popup/tooltip geometry and keyboard interaction (OverlayModel), scroll offset clamping (ScrollModel), split-pane drag ratio, and a minimal overlay paint path. Does NOT cover buttons, labels, icons, list items, toggles, dividers, keybinding display, or any colour/size/spacing scale. |
| `/Users/yoshinori/work/nimculus/src/nimnui/render.nim` | 170 | Paint command vocabulary. Has semantic kinds (workspacePanel, scrollbarTrack, editorActiveLine) which is a theme-aware move, but no shadow blur/alpha, no stroked path or dash array, and no per-command colour — so elevation shadow stacks and dashed dividers cannot be expressed. |

### メカニズム

#### Semantic colour token enum — 一部のみ

Zed: `crates/ui/src/styles/color.rs:19 (enum), :90 (Color::color)` / Nimculus: `src/nimculus/settings.nim:31 (ThemeColors, ~48 string fields); src/nimnui/platform/macos/macos_platform.m:942 (themeRoleColor), :908 (themeTokenFallback), :842 (themeHexColor)`
...
#### Style × state -> concrete style resolution — 無い

Zed: `crates/ui/src/components/button/button_like.rs:125 (ButtonStyle), :190 (ButtonLikeStyles), :210 enabled(), :257 hovered(), plus active()/disabled() through :448` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:957 (updateChromeButtonAppearance), :1005 (styleWorkspaceNavigationButton), :1053, :1059`
...
#### Size scale as a pure table — 一部のみ

Zed: `crates/ui/src/components/button/button_like.rs:455 (ButtonSize), :465 (rems); crates/ui/src/components/icon.rs:54 (IconSize), :70 (rems), :86 (square_components); crates/ui/src/styles/typography.rs:93 (TextSize), :132 (rems/pixels); crates/ui/src/components/label/label_like.rs:7 (LabelSize)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:438-473 (NimculusSpace1/2/3=4/8/12, NimculusRowHeight=28, NimculusControlHit=24, NimculusIconPointSize=14, NimculusToolbarIconPointSize=11, NimculusDefaultRemSize=16), :675 (NimculusUiTextSizePixels=14)`
...
#### Density-aware spacing scale — 無い
...
#### Elevation -> shadow stack and background — 一部のみ

Zed: `crates/ui/src/styles/elevation.rs:14 (ElevationIndex), :42 (shadow), :84 (bg), :95 (on_elevation_bg), :108 (darker_bg); crates/ui/src/traits/styled_ext.rs:6 (elevated), :45 elevation_1, :62 elevation_2, :83 elevation_3` / Nimculus: `src/nimnui/controls.nim:222-225 (paintOverlay: single hardcoded shadow offset (2,3), radius 6, one border); src/nimculus/settings.nim:36-38 (surface/elevated/panel tokens exist)`
...
#### Builder traits shared across components — 無い

Zed: `crates/ui/src/traits/clickable.rs:4, disableable.rs:2, toggleable.rs:5, fixed.rs, visible_on_hover.rs; crates/ui/src/components/button/button_like.rs:12 (SelectableButton), :17 (ButtonCommon)` / Nimculus: `none found`
...
#### Component-instance identity and state — 一部のみ

Zed: `crates/ui/src/components/button/button_like.rs:484 (ElementId), :512 (focus_handle), :745 render; crates/ui/src/components/context_menu.rs:211 (ContextMenu as an Entity with focus_handle, selected_index, subscriptions)` / Nimculus: `src/nimnui/ui_tree.nim:6 (NodeId), :12 (NodeHandle with generation), :16 (UiState normal/focused/hovered/active/disabled), :34 (independent focusedState/hoveredState/activeState/disabledState flags), :88 (updateVisualState precedence)`
...
#### Label rendering: size, weight, truncation mode — 一部のみ

Zed: `crates/ui/src/components/label/label_like.rs:7 (LabelSize), :23 (LineHeightStyle), :34 (LabelCommon), :233 (render)` / Nimculus: `src/nimnui/text.nim (467 lines, not read in detail); src/nimnui/platform/macos/macos_platform.m:4676-4682 (NSLineBreakByTruncatingTail paragraph style), :4757-4767 (manual width arithmetic to reserve room for a shortcut before truncating the title)`
...
#### Keybinding display — 無い

Zed: `crates/ui/src/components/keybinding.rs:46 (KeyBinding), :63 for_action, :200 render, :252 render_keybinding_keystroke, :411 (Key), :457 (KeyIcon)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:4685 (commandShortcut — a static NSDictionary literal mapping ~24 command names to pre-composed strings like @"⌘⇧P"), :4757 (the row draws that string right-aligned)`
...
Zed: `crates/ui/src/traits/toggleable.rs:12 (ToggleState), :26 inverse, :34 from_any_and_all; crates/ui/src/components/toggle.rs:43 (Checkbox), :181 container_size, :338 (Switch), :328 (SwitchLabelPosition)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:4448 (gitCheckboxes: NSMutableArray<NSButton*> — plain AppKit checkboxes, which do support NSControlStateValueMixed but I found no code setting it)`
...
Zed: `crates/ui/src/components/divider.rs:19 (DividerColor), :37 (struct), :96 render_solid, :100 render_dashed` / Nimculus: `src/nimnui/render.nim:160 (drawWorkspaceSeparator — a semantic paint kind), :104 drawBorder; src/nimnui/platform/macos/macos_platform.m:444 (NimculusChromeBorderHeight = 1.0)`
...
#### Tooltip container as shared chrome — 一部のみ

Zed: `crates/ui/src/components/tooltip.rs:216 (tooltip_container), :194 (Tooltip::render), :9 (struct: title, meta, key_binding)` / Nimculus: `src/nimnui/controls.nim:132 (showTooltip), :226-229 (paintOverlay tooltip branch: 4/8/4/8 insets, radius 6, one shadow); src/nimnui/platform/macos/macos_platform.m:4161, :4241 (NSView.toolTip — the system tooltip, not a custom one)`
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/workspace/src/workspace.rs` | 17052 | Owns the Workspace entity: the center PaneGroup, the three Docks, status bar, modal/toast layers, titlebar slot, follower state, and the top-level render that stacks titlebar / docks+center / status bar, plus workspace serialization scheduling. |
| `crates/workspace/src/pane.rs` | 9455 | The Pane: an ordered list of Box<dyn ItemHandle>, active index, activation history, pinned prefix, preview item, nav history, zoom flag, toolbar, and the tab-bar/tab rendering and drag-drop. |
| `crates/workspace/src/persistence.rs` | 5954 | SQLite-backed workspace database; the row shapes live in persistence/model.rs (SerializedWorkspace, DockStructure/DockData, SerializedPaneGroup/Pane/Item) and their deserialize-into-live-entities routines. |
| `crates/workspace/src/item.rs` | 1869 | The Item trait (tab content, tab icon, tooltip, dirty/conflict, save, breadcrumbs) plus its object-safe ItemHandle, SerializableItem, ProjectItem and FollowableItem, and ItemSettings/PreviewTabsSettings. |
| `crates/workspace/src/pane_group.rs` | 1608 | The recursive split tree: Member = Axis(PaneAxis) | Pane, PaneAxis with per-child flexes and cached bounding boxes, split/remove/resize/swap, SplitDirection, and the handle hitbox constant. |
| `crates/workspace/src/dock.rs` | 1558 | The Panel trait and its object-safe PanelHandle, the Dock container (one per edge) with panel entries, per-panel size state, open/active/zoom bits, resize handle rendering, and PanelButtons for the status bar. |

### Nimculus の対応物

| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m` | 17976 | NimculusTabBarOverlay at line 4460 — the actual tab strip, with tab rects, hit testing, drag-reorder, back/forward/new/split/zoom buttons. This is render_tab_bar and render_tab, reimplemented in AppKit and driven by a newline-joined title string. |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | The wiring: workspace layout application (main.nim:400-580), breadcrumb construction (main.nim:1283-1400), tab-bar sync to the native strip (main.nim:5326-5340), split commands (main.nim:4260, 6914), panel toggle commands (main.nim:8079-8191). Everything Zed puts in Workspace::render and the Pane's action handlers lives here. |
| `/Users/yoshinori/work/nimculus/src/nimculus/workspace.nim` | 853 | Despite the name this is NOT Zed's workspace crate — it is the project/worktree layer: roots, ignore rules, directory listing, file CRUD, fuzzy and content search. It corresponds to Zed's `project` crate, not `workspace`. |
| `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim` | 841 | Dock state and toggling, panel-to-dock ownership and settings reconciliation, dock resize logic, workspace root layout and hit testing, the pane split tree and its layout/minimums, panel list selection, and dock save/restore. This is the real counterpart to dock.rs + pane_group.rs + the layout half of workspace.rs. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_app.nim` | 553 | EditorSession and EditorTab: the item list, active index, pinned prefix, closed-tab stack, tab move/switch, per-tab view state. This is the item-ownership half of pane.rs plus a thin Item. |
| `/Users/yoshinori/work/nimculus/src/nimculus/session.nim` | 322 | JSON persistence of the session: dock open/size/panel triples, the flat split scalars, and the tab list with per-tab view state and dirty-content recovery. Counterpart to persistence/model.rs, minus the recursive pane group. |
| `/Users/yoshinori/work/nimculus/src/nimculus/status_bar.nim` | 84 | Status-bar footer item composition. Partial counterpart to StatusItemView; the dock PanelButtons equivalent is the packed mask in workspace_ui.nim consumed by AppKit. |

### メカニズム

#### Panel trait (what a dock can contain) — 一部のみ

Zed: `crates/workspace/src/dock.rs:36` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:16 (PanelKind enum), :364 panelPositionIsValid, :330 panelDockSettingKey, :350 panelDockSide`
...
#### PanelHandle (object-safe erasure of Panel) — 無い
...
#### Dock container: open bit, active panel index, per-panel size state — 一部のみ
...
#### DockPosition and its axis — 移植済み

Zed: `crates/workspace/src/dock.rs:290, axis() at dock.rs:335` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:21 DockSide, :24 DockAxis, :142 axis()`
...
#### Item trait (what a tab knows how to be) — 一部のみ

Zed: `crates/workspace/src/item.rs:170` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/editor_app.nim:25 EditorTab, :219 displayTitle, :233 tabDisplayLabel; breadcrumb construction at /Users/yoshinori/work/nimculus/src/nimculus/main.nim:1361`
...
#### Pane: item list, active index, activation history, pinned prefix, preview item — 一部のみ

Zed: `crates/workspace/src/pane.rs:398` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/editor_app.nim:47 EditorSession (tabs/activeTab), :240 pinnedTabCount, :265 setTabPinned; per-pane indices at /Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:32 PaneState`
...
#### Nav history (back/forward across items) — 無い
...
#### Tab bar rendering (tabs, nav buttons, pinned row, drop targets) — 一部のみ

Zed: `crates/workspace/src/pane.rs:3396 render_tab_bar, per-tab at pane.rs:2825 render_tab, drop target at pane.rs:3626` / Nimculus: `AppKit-side: /Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m:4460 NimculusTabBarOverlay (tabRectForIndex, tabIndexAtPoint, dispatchTabMoveFrom:to:, back/forward/tabList/new/split/zoom buttons); fed from /Users/yoshinori/work/nimculus/src/nimculus/main.nim:5326-5334 platformSetEditorTabs`
...
#### Item toolbar / breadcrumb slot — 一部のみ

Zed: `crates/workspace/src/toolbar.rs:64 Toolbar, ToolbarItemLocation at toolbar.rs:56; Item side at item.rs:343 breadcrumb_location, :347 breadcrumbs` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/main.nim:1361 (breadcrumb segment construction with NativeBreadcrumbHighlight), :1290 breadcrumbSymbolsAtCursor, :1304 markdownBreadcrumbHeadings, geometry at main.nim:434 EditorBreadcrumbHeight = 45`
...
| `crates/editor/src/display_map.rs` | 4356 | Composes the five transform layers into one DisplayMap/DisplaySnapshot and layers text/inlay/semantic-token highlights on top; defines DisplayPoint/DisplayRow, the display coordinate space the element paints in. |
...
| `crates/editor/src/display_map/custom_highlights.rs` | 421 | Merges arbitrary anchored highlight ranges into the chunk stream the layers produce. |
| `crates/editor/src/display_map/invisibles.rs` | 133 | Classifies invisible characters and supplies their visible replacement glyphs. |
| `crates/editor/src/scroll/autoscroll.rs` | 0 | Autoscroll strategies (Fit/Newest/Center/Top/Bottom/TopRelative) and the vertical/horizontal autoscroll application. |
| `crates/editor/src/scroll/scroll_amount.rs` | 0 | ScrollAmount: Line/Page/Column/PageWidth, converted to lines, columns or pixels. |

### Nimculus の対応物

| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | Editor action dispatch, LSP inlay hint state (:167-172, :5513, :5536), soft-wrap scroll reconciliation (:5249-5260), selection publishing to the native renderer (:5309, :5433), and the caret/IME placement — i.e. the Editor struct's behavioural half, with no Editor type. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_app.nim` | 553 | FileDocument, EditorTab (owning per-tab view + secondaryView), EditorSession with tabs/splits/recents, and ensureCursorVisible (:524) which is Nimculus's entire autoscroll. This file mixes Zed's editor-item layer with Zed's workspace layer. |
...
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_view.nim` | 331 | Per-view state: selections (primary + additional), scroll position in pixels/lines, soft-wrap and line-number flags, folded ranges, word/grapheme movement, wheel delta conversion, status text. Corresponds to a flattened merge of Zed's SelectionsCollection, parts of ScrollManager, and the fold/wrap *settings* of DisplayMap. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_buffer.nim` | 326 | Buffer text storage (piece table), edits, undo/redo transactions with content versioning, UTF-8/UTF-16/grapheme position conversion, line index. Covers Zed's text::Buffer role for a single file; covers none of MultiBuffer or Anchor. |
...
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_text_layout.nim` | 230 | The whole display pipeline compressed into three procs: buildVisibleEditorLayout (:132), displayRowsBeforeLine (:172), sourceLineForDisplayRow (:205). Handles wrapping via a per-line layout cache and folds via a per-line skip test; no inlay, tab, or block transform, no snapshot, no incremental sync. |
...
#### MultiBuffer: N buffers presented as one text — 無い
...
#### Anchor: an edit-surviving position — 無い

Zed: `crates/multi_buffer/src/anchor.rs:17 (ExcerptAnchor), :28 (enum Anchor {Min, Excerpt, Max})` / Nimculus: `src/nimculus/editor_buffer.nim:13 (Selection is two raw ints)`
...
#### The five-layer transform chain (inlay → fold → tab → wrap → block) — 無い

Zed: `crates/editor/src/display_map.rs:1 (module doc describing the contract), :213 (DisplayMap fields), :604 (snapshot() driving the chain)` / Nimculus: `src/nimculus/editor_text_layout.nim:132 (buildVisibleEditorLayout collapses all of it into one loop)`
...
Zed: `crates/editor/src/display_map/inlay_map.rs:34 (InlayMap), :40 (InlaySnapshot), :574 (sync)` / Nimculus: `src/nimculus/main.nim:167-172 (editorLspInlayHints), :5513 syncNativeInlayHints`
...
#### Fold layer and creases — 一部のみ

Zed: `crates/editor/src/display_map/fold_map.rs:360 (FoldMap), :680 (FoldSnapshot), :1225 (Fold), :1232 (FoldRange over Anchors), :27 (FoldPlaceholder); crates/editor/src/display_map/crease_map.rs:15` / Nimculus: `src/nimculus/editor_view.nim:39 (foldedRanges: seq[FoldRange]), src/nimculus/syntax.nim:12 (FoldRange), :185 foldRanges`
...
#### Soft wrap layer with background rewrap — 一部のみ

Zed: `crates/editor/src/display_map/wrap_map.rs:32 (WrapMap with pending_edits, interpolated_edits, background_task), :43 (WrapSnapshot), :146 (sync)` / Nimculus: `src/nimculus/editor_text_layout.nim:103 addWrappedRows, :160 cache.layoutWrappedLineByHash; src/nimculus/editor_view.nim:34 (softWrap flag)`
...
#### Block layer: non-text rows and replacement blocks — 無い

Zed: `crates/editor/src/display_map/block_map.rs:38 (BlockMap), :75 (BlockSnapshot), :163 (BlockPlacement Above/Below/Near/Replace), :377 (enum Block: Custom, FoldedBuffer, ExcerptBoundary, BufferHeader, Spacer), :282 (BlockProperties), :303 (BlockStyle)` / Nimculus: `src/nimculus/editor_diagnostics.nim (23 lines, no block model)`
...
#### ScrollAmount: line / page / column / page-width — 一部のみ

Zed: `crates/editor/src/scroll/scroll_amount.rs:19 (enum ScrollAmount), :30 lines(), :46 columns(), :53 pixels()` / Nimculus: `src/nimculus/editor_view.nim:257 scrollPixelDelta, :267 scrollLineDelta`
...
#### Invisible character rendering — 無い
...
#### EditorMode: one Editor type, several shapes — 無い
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/editor/src/element.rs` | 12296 | The whole editor element: implements gpui's Element for EditorElement, doing request_layout / prepaint (all layout_* fns producing an EditorLayout) and paint (all paint_* fns), plus the line-shaping type LineWithInvisibles, CursorLayout, ScrollbarLayout, PositionMap and render_breadcrumb_text. |
| `crates/editor/src/items.rs` | 3286 | Implements workspace's Item/FollowableItem/SerializableItem/ProjectItem for Editor: tab title text, tab_content element, tab icon/tooltip, breadcrumb segments and breadcrumb prefix, and the git/diagnostic-aware label colour helpers. |
| `crates/editor/src/git.rs` | 3148 | Editor-side git state and behaviour: the DiffHunkDelegate trait and its three implementations, diff hunk resolution/staging/restore, and the editor's blame lifecycle (toggle_git_blame, start_git_blame, blame popover). |
| `crates/editor/src/git/blame.rs` | 1330 | The GitBlame entity: a SumTree of blame entries kept in sync with buffer edits, blame_for_rows for a row window, max_author_length, and the BlameRenderer trait plus its global registration. |
...
| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimnui/platform/macos/macos_platform.m` | 17976 | Everything Zed's paint half does, plus GutterDimensions (:2207) which in Zed is editor state. Metal paint kinds for caret (:2494), selection (:2500/:2578), scrollbar thumb (:2507), scrollbar track rule (:2565); AppKit overlays for line numbers and diff bars (:6414), indent guides (:6510), inlay annotations (:8817), inline blame (:8794). |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | The prepaint driver (buildVisibleEditorLayout call and FFI publish at :4144-4181), the breadcrumb payload builder (:1360), the git gutter mouse route (:2906), and the blame cache lifecycle (:4985-5008). This is Zed's EditorElement::prepaint plus parts of items.rs, inlined into the application main. |
| `/Users/yoshinori/work/nimculus/src/nimculus/syntax.nim` | 398 | The chunk producer feeding from_chunks: HighlightSpan/HighlightKind (10 kinds) and highlightVisible for a byte window, plus fold ranges, outline items and bracket matching. Corresponds to Zed's highlighted_chunks source rather than to element.rs itself. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_view.nim` | 331 | Per-view paint-relevant state: selections, scroll position (pixel and line), soft wrap flag, indent guides, folded ranges, line height. This is Zed's per-editor state that EditorElement reads during prepaint. |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_text_layout.nim` | 230 | LineWithInvisibles::from_chunks (font runs per line, wrapped row splitting, glyph positioning), x_for_index (as xAt), layout_lines' visible-window discipline, inline blame start-x. Also carries the display-row↔source-line mapping that Zed puts in the display map, not the element. |
...
| `/Users/yoshinori/work/nimculus/src/nimculus/git_blame.nim` | 67 | A flat, version-keyed cache standing in for the GitBlame entity, plus the shouldStart/shouldShow gating predicates. No SumTree, no edit splicing, no row-window iterator. |
...
#### gutter_strip_width — the diff change-bar width — 無い

Zed: `crates/editor/src/element.rs:5309` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:6494 draws a fixed 2.0pt-wide bar at x=(left_padding-2)/2 with height line_height-4`
...
#### diff_hunk_bounds — hunk vertical extent in gutter space — 無い
...
#### layout_line_numbers — number choice, colour, placement, hit target — 一部のみ

Zed: `crates/editor/src/element.rs:2727; LineNumberStyle at crates/editor/src/element.rs:115-141` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:6414-6503 (NimculusLineNumberOverlay drawRect)`
...
#### x_for_index / index_for_x / alignment_offset — line-local pixel↔column mapping — 一部のみ

Zed: `crates/editor/src/element.rs:7661, crates/editor/src/element.rs:7690, crates/editor/src/element.rs:7745` / Nimculus: `src/nimculus/editor_text_layout.nim:97 (xAt)`
...
#### calculate_wrap_width — soft wrap mode to a pixel width — 一部のみ

Zed: `crates/editor/src/element.rs:10624, consumed at crates/editor/src/element.rs:8023 and :10672` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:3062-3065 (the two-em reservation, with a comment citing Zed), src/nimculus/editor_text_layout.nim:132 takes wrapWidth as a caller-supplied parameter`
...
#### layout_selections + active_rows — 一部のみ

Zed: `crates/editor/src/element.rs:767, SelectionLayout at crates/editor/src/element.rs:142 and :159` / Nimculus: `src/nimculus/editor_view.nim:13-18 (selection + additionalSelections), src/nimnui/platform/macos/macos_platform.m:311-312 (g_editor_selections array, NIMCULUS_MAX_EDITOR_SELECTIONS)`
...
#### CursorLayout — shape, bounds, block glyph, collaborator name — 一部のみ

Zed: `crates/editor/src/element.rs:10318 (struct), crates/editor/src/element.rs:10362 (bounds), crates/editor/src/element.rs:10421 (paint); construction at crates/editor/src/element.rs:1001 (layout_visible_cursors)` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:2494 (paint kind 8, caret)`
...
#### layout_inline_blame — x placement of the end-of-line blame text — 移植済み

Zed: `crates/editor/src/element.rs:2008` / Nimculus: `src/nimculus/editor_text_layout.nim:27 (inlineBlameStartX), consumed at src/nimnui/platform/macos/macos_platform.m:8851-8857`
...
#### GitBlame entity — blame kept in sync with edits — 一部のみ

Zed: `crates/editor/src/git/blame.rs:75 (struct GitBlame), :305 (blame_for_rows), :368 (sync), :505 (generate), :681 (regenerate_on_edit), :697 (build_blame_entry_sum_tree)` / Nimculus: `src/nimculus/git_blame.nim:10 (GitBlameCache, a flat seq[GitBlameLine] keyed by {root, path, documentVersion})`
...
#### paint_background — active line and highlighted rows — 一部のみ

Zed: `crates/editor/src/element.rs:4890` / Nimculus: `src/nimnui/platform/macos/macos_platform.m:6435-6438 notes the active-line rectangle is painted by Metal below the line-number overlay`
...
| ファイル | 行 | 役割 |
| --- | ---: | --- |
| `crates/project/src/lsp_store.rs` | 15529 | Owns language server processes, their lifecycle state, per-buffer LSP document synchronization and snapshot history, diagnostics storage and summaries, and per-buffer caches of LSP-derived data (inlay hints, colors, code lens, semantic tokens). |
| `crates/project/src/git_store.rs` | 10737 | Owns git Repository entities, per-buffer diff bases (head/index text) and their recalculation, the serialized git job queue, blame, status scanning and the commit-data cat-file batch process. |
| `crates/worktree/src/worktree.rs` | 7199 | The filesystem model: Snapshot/LocalSnapshot of entries in SumTrees, the BackgroundScanner that watches the fs and produces scan-id-versioned snapshots plus UpdatedEntriesSet deltas, and detection of git repositories inside the tree. |
| `crates/project/src/project.rs` | 6847 | The Project entity: owns one instance of every store (worktree/buffer/lsp/git/dap/image/task/toolchain), subscribes to each store's event stream, and re-emits a single unified project::Event that the workspace/editor UI observes. |
| `crates/buffer_diff/src/buffer_diff.rs` | 4315 | The BufferDiff entity: hunks stored anchored to the buffer in a SumTree, hunk status kind/secondary(staged) status, pending-hunk overlay, and range/row queries used by the editor gutter. |
...
| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | Everything Zed puts in Project itself: the store instances (main.nim:989-1034), the subscription/event wiring (replaced by per-frame polling), the git status→panel projection (:2591-2709), git hunk scheduling (:3985-4070), gutter action dispatch (:2903-2913) and repository selection policy (:1401). |
| `/Users/yoshinori/work/nimculus/src/nimculus/lsp.nim` | 1134 | LSP transport: framing, request tracking with timeouts, message construction and parsing for ~15 requests, diagnostics table, work-done progress. Corresponds to Zed's lsp crate plus the protocol half of lsp_store.rs; contains no server lifecycle or multi-server routing. |
| `/Users/yoshinori/work/nimculus/src/nimculus/workspace.nim` | 853 | Roots, entry listing, ignore stacks, fs watching and change coalescing, fuzzy and text search jobs. Corresponds to worktree.rs plus worktree_store.rs plus project_search.rs, with no snapshot versioning and no entry ids. |
| `/Users/yoshinori/work/nimculus/src/nimculus/lsp_editor_bridge.nim` | 788 | Per-feature request/response state machine and document open/change/close tracking for one server. Corresponds to LspStore's BufferLspData and the buffer registration path, collapsed into one object with a field per feature. |
| `/Users/yoshinori/work/nimculus/src/nimculus/git_service.nim` | 567 | Repository resolution and caching, the async GitJob process wrapper, status/diff/branch/log/blame/commit command surface and their parsers. Covers the command layer of Zed's git_store.rs but none of its state layer (no Repository entity, no snapshot, no job queue, no diff bases). |
...
| `/Users/yoshinori/work/nimculus/src/nimculus/git_blame.nim` | 67 | Blame caching keyed by (repo root, path, document version) with an explicit negative-result state. No Zed counterpart at this granularity; Zed caches blame in the editor's BlameRenderer instead. |
| `/Users/yoshinori/work/nimculus/src/nimculus/git_gutter.nim` | 33 | Gutter hit-testing and pointer-to-line resolution for stage/unstage. Corresponds to editor-layer gutter geometry rather than to project/src, and is deliberately pure so input and git behavior cannot drift. |
...
#### Entry identity: ProjectEntryId, inode-based rename detection — 無い

Zed: `crates/worktree/src/worktree.rs:3896 (struct Entry, fields id/inode/mtime/is_ignored/is_hidden/is_private/is_external), :292 (RemovedEntries with by_inode and by_path)` / Nimculus: `src/nimculus/workspace.nim:22 (WorkspaceEntry has only path/relativePath/rootPath/kind/ignored)`
...
#### Off-thread work with a main-thread completion boundary — 移植済み

Zed: `crates/project/src/git_store.rs:512-545 (LocalRepositoryState::new using cx.background_spawn), crates/worktree/src/worktree.rs:424 (UpdateObservationState channels), :410 (ScanState delivered over an UnboundedSender)` / Nimculus: `src/nimculus/git_service.nim:180 newGitRepository (BackgroundExecutor.spawn + Future callback), :292 poll (non-blocking process poll), src/nimculus/workspace.nim:827 receiveWorkspaceChange (locked queue drained on the main thread), src/nimculus/lsp_editor_bridge.nim:608 poll`
...
| `crates/zed/src/zed.rs` | 7250 | Turns a bare Workspace into the Zed workspace: `initialize_workspace` (429), `initialize_panels` (775), `initialize_agent_panel` (874), `initialize_pane` (1402) — i.e. which panels get constructed and docked at startup. |
...
| `crates/extensions_ui/src/extensions_ui.rs` | 2096 | The extensions store/browser UI. |
| `crates/debugger_ui/src/debugger_panel.rs` | 2012 | Debug session panel (stack, variables, console): `impl Panel for DebugPanel` (1541). |
| `crates/zed/src/main.rs` | 2004 | The single wiring point: calls every feature crate's `init(cx)` in a fixed order (lines 491-771), which is how features attach themselves to the global App without the binary knowing their internals. |
| `crates/extension_host/src/extension_host.rs` | 1995 | Downloads, installs and runs extensions (wasm), and proxies their capabilities into language/theme/dap registries. |
| `crates/picker/src/picker.rs` | 1898 | The generic filtered-list modal that command_palette, file_finder, tab_switcher, outline, project_symbols, theme/language/toolchain selectors are all thin delegates of. |
| `crates/workspace/src/dock.rs` | 1558 | Defines the `Panel` trait (36) and its object-safe `PanelHandle` (98) plus `DockPosition` (290); this is the contract every feature panel crate implements to live in a dock. |
| `crates/title_bar/src/title_bar.rs` | 1446 | Window title bar with project/branch/collab controls. |
| `crates/command_palette/src/command_palette.rs` | 1326 | Action palette over the registered action namespace, a Picker delegate. |
| `crates/diagnostics/src/diagnostics.rs` | 1154 | Project-wide diagnostics as a multibuffer editor Item, not a panel. |
| `crates/tasks_ui/src/tasks_ui.rs` | 625 | Task spawn/rerun modal over the `task` crate's resolved task templates. |
| `crates/search/src/search.rs` | 247 | Shared search option/toggle vocabulary; the real work is buffer_search.rs, project_search.rs, search_bar.rs in the same crate. |
| `crates/repl/src/repl.rs` | 75 | REPL/Jupyter-kernel feature entry (`init` at 31); notebook UI is repl::notebook, initialised separately in main.rs:733. |
| `crates/collab_ui/src/collab_ui.rs` | 62 | Collaboration UI entry (channels, calls, shared screens); backed by collab, call, channel, livekit_client. |

### Nimculus の対応物

| ファイル | 行 | 何を担っているか |
| --- | ---: | --- |
| `/Users/yoshinori/work/nimculus/src/nimculus/main.nim` | 9845 | Almost every feature crate's *view* half at once: panel rendering, quick open (4707), command palette, workspace search (4663), diagnostics painting (286), breadcrumbs (434), scrollbars (1188), session restore (4237), native shortcut dispatch (663). Corresponds to zed/src/zed.rs plus the UI half of a dozen feature crates. |
...
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_view.nim` | 331 | per-view transient UI state including modal flags (commandPaletteOpen at :40) — where a Picker abstraction would live |
| `/Users/yoshinori/work/nimculus/src/nimculus/update_service.nim` | 327 | crates/auto_update |
| `/Users/yoshinori/work/nimculus/src/nimculus/session.nim` | 322 | crates/session + workspace persistence |
| `/Users/yoshinori/work/nimculus/src/nimculus/extension_service.nim` | 298 | crates/extension manifest + permission validation |
| `/Users/yoshinori/work/nimculus/src/nimnui/commands.nim` | 228 | the action/keymap dispatch that command_palette, vim, and which_key would all build on (context predicates at :42, resolve at :48) |
| `/Users/yoshinori/work/nimculus/src/nimculus/wasm_runtime.nim` | 224 | extension_host execution half, via an external wasm runtime process |
| `/Users/yoshinori/work/nimculus/src/nimculus/windows_terminal.nim` | 202 | terminal platform backend for Windows |
| `/Users/yoshinori/work/nimculus/src/nimculus/task_service.nim` | 195 | crates/task runner half; tasks_ui absent |
| `/Users/yoshinori/work/nimculus/src/nimculus/extension_catalog.nim` | 154 | extension download/verify/install (extension_host install half) |
| `/Users/yoshinori/work/nimculus/src/nimculus/status_bar.nim` | 84 | workspace status bar item content |
| `/Users/yoshinori/work/nimculus/src/nimculus/git_blame.nim` | 67 | inline blame cache (git_ui blame) |
| `/Users/yoshinori/work/nimculus/src/nimculus/search.nim` | 54 | search/buffer_search matcher only |
| `/Users/yoshinori/work/nimculus/src/nimculus/git_gutter.nim` | 33 | diff hunk gutter hit-testing (git_ui gutter) |
| `/Users/yoshinori/work/nimculus/src/nimculus/editor_diagnostics.nim` | 23 | crates/diagnostics, inline only |

### メカニズム

#### Crate init() registration order — 無い
...
#### Panel persistence identity (persistent_name) — 一部のみ

Zed: `crates/project_panel/src/project_panel.rs:7607, crates/outline_panel/src/outline_panel.rs:4955, crates/terminal_view/src/terminal_panel.rs:1625` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:17-19 ("Keep new values at the end: panel ordinals are persisted in sessions"), :185 panelFromOrdinal, :297 saveWorkspaceUi`
...
#### Startup panel construction — 一部のみ

Zed: `crates/zed/src/zed.rs:775 initialize_panels, :874 initialize_agent_panel` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:206 initWorkspaceUi(session, settings), :234 applyPanelDockSettings`
...
#### Project panel (file tree) — 移植済み

Zed: `crates/project_panel/src/project_panel.rs:464 init, :7567 impl Panel` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace.nim:159 listChildrenAt, :272-353 create/delete/rename/copy; rendered from /Users/yoshinori/work/nimculus/src/nimculus/main.nim:4322 refreshWorkspacePreview, :6329 openFilesDockEntry`
...
Zed: `crates/outline_panel/src/outline_panel.rs:653 init, :4954 impl Panel` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/workspace_ui.nim:18 (panelOutline exists); symbol source at /Users/yoshinori/work/nimculus/src/nimculus/lsp.nim`
...
#### Search: buffer search bar vs project search item — 一部のみ

Zed: `crates/search/src/search.rs:1-247 plus buffer_search.rs, project_search.rs, search_bar.rs, text_finder.rs (crate 12698 lines)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/search.nim:24 findMatches (in-buffer), /Users/yoshinori/work/nimculus/src/nimculus/workspace.nim:491 startSearch/:546 pollSearch/:715 searchRipgrep (project), UI at /Users/yoshinori/work/nimculus/src/nimculus/main.nim:4663 renderWorkspaceSearch, :4876 syncDocumentSearchUi`
...
#### Vim modal editing as an editor addon — 無い
...
#### Picker: one modal, many delegates — 一部のみ

Zed: `crates/picker/src/picker.rs (1898 lines); delegates in file_finder/src/file_finder.rs:1, command_palette/src/command_palette.rs:1, tab_switcher, outline, project_symbols, theme_selector, language_selector, toolchain_selector, recent_projects, go_to_line` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/editor_view.nim:40 commandPaletteOpen, :326 openCommandPalette; quick-open at /Users/yoshinori/work/nimculus/src/nimculus/main.nim:4707 renderQuickOpen, :4775 showQuickOpen; fuzzy matcher at /Users/yoshinori/work/nimculus/src/nimculus/workspace.nim:385 fuzzyScore, :370 startFuzzySearch`
...
#### Diagnostics as a multibuffer Item — 一部のみ

Zed: `crates/diagnostics/src/diagnostics.rs (1154 lines), init at crates/zed/src/main.rs:734` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/editor_diagnostics.nim:12 resolveDiagnostics (23 lines), inline rendering at /Users/yoshinori/work/nimculus/src/nimculus/main.nim:286-318`
...
#### Task templates and spawn UI — 一部のみ

Zed: `crates/tasks_ui/src/tasks_ui.rs (625 lines), init at crates/zed/src/main.rs:747; model in crates/task` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/task_service.nim:128 startTask, :94 parseTaskProblems, :49 appendBoundedTaskOutput; panelTasks slot at workspace_ui.nim:18`
...
#### Collaboration (collab, collab_ui, call, channel, livekit) — 無い
...
#### Auxiliary viewers (image_viewer, markdown_preview, svg_preview, csv_preview) — 無い
...
Zed: `crates/auto_update init at crates/zed/src/main.rs:659, auto_update_ui at :661` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/update_service.nim:143 parseUpdateManifest, :44 macosUpdateMountedAppPath, :140 isHttpsUrl, :36 artifactWithinLimit`
...
#### Status bar, title bar, breadcrumbs, activity indicator — 一部のみ

Zed: `crates/title_bar/src/title_bar.rs (1446 lines), crates/breadcrumbs (127), crates/activity_indicator (781), crates/notifications (684)` / Nimculus: `/Users/yoshinori/work/nimculus/src/nimculus/status_bar.nim:30 statusBarFooter, :78 gitBlameStatusText; breadcrumb geometry at /Users/yoshinori/work/nimculus/src/nimculus/main.nim:434 EditorBreadcrumbHeight, :524-529`
