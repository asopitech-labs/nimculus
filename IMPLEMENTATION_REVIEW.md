...
- M11 release gate: `NIMCULUS_REQUIRE_NOTARIZATION=1` now validates stapled tickets and Gatekeeper for both the mounted app and DMG; `.github/workflows/macos-release.yml` provides the credential-backed manual Developer ID/App Store Connect release path without committing secrets.
- M11 cache boundary: Swift ICNS generation now places `CLANG_MODULE_CACHE_PATH` below the per-package temporary cache. A local adhoc package smoke exposed and then verified the fix; no user-home Clang module cache is required.
- M11 update cancellation: active `curl` downloadを終了要求でcancelし、1秒grace＋kill fallbackでpartial/destinationを回収する。更新用のhash/signature/DMG/rsync runnerも60秒timeoutを持つ。疑似`curl`でbounded cancellationを回帰テスト済み。
- M11 DMG mount contract: `hdiutil -mountroot`の実際の`<mountroot>/Nimculus`ボリューム経路をmounted app検証・rsync・detachへ統一。package scriptの`-volname Nimculus`と同じ契約をunit testで固定した。
...
- M1 window state contract: `NSWindowCollectionBehaviorFullScreenPrimary`とAppKit標準のminiaturize/zoom actionを本番window/menuへ設定し、native contractでfullscreen capability、window state mask、接続済み全モニターの境界、Retina drawable resizeを確認する。物理的な複数ディスプレイ移動の手動確認は未完了。
...
- M1 input field contract: native smoke now exercises AppKit mouse, key, flagsChanged, CGEvent-backed precise scroll, mouseEntered, and mouseExited events through the field-reading path; event-specific keyCode access remains guarded.
...
- M2 overlay controls: `ControlKind`だけだったContext menu / Popup / Tooltipに、anchor-relative viewport placement、上下flip、画面内clamp、disabled/separator skip、keyboard selection、activation、outside-click dismiss、passive tooltip exit、PaintList出力を追加。macOS実機での見た目・操作確認は未完了。
...
- M1 retained scene resize/damage: 新しいdrawableサイズ・Metal deviceでは保持sceneを未初期化へ戻し、初回／再生成後は古いdirty regionを無視してPaintList、glyph、text overlayを全量再構築するnative contractを追加。既存sceneでのみdamage scissorを使う。
...
- M6 search cancellation: ripgrepのキャンセルと32 MiB出力上限超過で、1秒の終了猶予後に強制終了する有限プロセス境界を追加。`external search termination is bounded`テストで10秒待ちの子プロセスがUI停止時間内に回収されることを確認。
- M6 Worktree metadata: macOS Workspaceプレビューから呼ばれる`git worktree list`、`rev-parse`、`symbolic-ref`を64 KiB上限・2秒timeout付きのプロセス runnerへ移し、失敗・timeout・切り詰め時は部分状態を表示しない。無応答Gitの回帰テストを追加。
...
- M6 FSEvents rescan boundary: Zed watcherの`need_rescan`契約を参照し、`MustScanSubDirs`、drop、event ID wrap、root changeを無視せず、監視ルートを一つのcache invalidation/rescan境界として通知するよう修正した。
...
- M8 request timeout: trackerに存在していた期限計算がsession pollへ接続されておらず、無応答Language Serverのpending requestが残り続ける漏れを修正。既定30秒で`$/cancelRequest`を送信し、initialize timeoutは再利用不能なsession failure、feature timeoutは個別requestの破棄として扱う。
- M8 LSP shutdown boundary: Language ServerがSIGTERMを無視した場合に`waitForExit()`がmacOS UIを無期限に停止させる漏れを修正し、terminate時は1秒のbounded wait後にkillへフォールバック。
- M9 Git shutdown boundary: Gitがstdin待ちやSIGTERM無視状態になった場合の無期限`waitForExit()`を、1秒のbounded waitとkill fallbackへ変更し、GitキャンセルをUIブロッキング操作にしない。
- M9 Git discovery/output boundary: repository probeを`GitJob`のnon-blocking drainへ統一し、応答しないGitを2秒で停止する。実行中stdoutを先にdrainしてfull pipe deadlockを避け、1 MiB出力の疑似Gitで回帰確認した。
- M10 process shutdown boundary: TaskJobのcancelとmacOS PTY closeに無期限waitが残っていたため、taskは1秒bounded wait＋kill fallback、PTY childは1秒grace period＋SIGKILL/reapへ統一。
...
- M5 external identity: Zedの`Metadata`が持つdevice/file identityを参照し、サイズ・mtimeだけでは見逃す同一サイズのatomic replacementを`FileDocument`でも検知。atomic temporary nameには同一プロセス内のsequenceを加え、連続保存で一時パスが衝突しないよう修正。
