...
- M3 atlas status: `GlyphAtlas`はNim側のモデルとunit testに留まり、native Metal側は現在もCore Textで可視文書全体をrasterizeするtexture経路。ロードマップのglyph atlas接続を未完了へ訂正し、実装済みと誤認しない。
...
完了したが、native glyph atlas接続、`Cmd+,`設定画面、Git diff/stage等の高度なUI、IMEの実機操作、カーソル/選択/文字表示のGUI確認は残っている。
