...
- M2: UIツリー、レイアウト、イベント、PaintList filtering、世代付きIDのunit test。Metal deviceが利用可能な実行環境でネイティブスモークを確認し、部分再描画のMetal接続とGUIギャラリー実機確認は未完了。
...
完了したが、Git diff/stage等の高度なUI、IMEの実機操作、カーソル/選択/文字表示のGUI確認は残っている。
