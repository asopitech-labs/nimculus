...
- M2: UIツリー、レイアウト、イベント、PaintList filtering、世代付きIDのunit test。PaintListの矩形コマンドをmacOS native ABI経由でMetalへ転送し、保持用scene textureをdirty領域だけ更新してdrawableへblitする経路を追加。GUIギャラリー実機確認は未完了。
