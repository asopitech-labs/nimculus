...
- M2: UIツリー、レイアウト、イベント、dirty paint、世代付きIDのunit test。ネイティブMetalスモークは確認済み、GUIギャラリー実機確認は未完了。
...
- Metal device、CAMetalLayer、Retina drawable sizeを確認するネイティブスモークテストを追加。
...
完了したが、M6検索のUI非ブロッキングサービス統合、大規模Tree-sitter計測、GUI実機操作確認は残っている。
