...
**実装済み基盤：** UTF-8 byte-accurate `Content-Length` framing、partial/multiple frame decoder、JSON-RPC request construction、method generationによるstale response破棄、cancel状態管理（`src/nimculus/lsp.nim`）。
