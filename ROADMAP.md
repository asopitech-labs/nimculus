...
**macOS実装状況：** Appleの`lldb-dap`を`NIMCULUS_DAP_COMMAND`指定なしでもXcode選択状態から`xcrun --find`で解決し、Content-Length transport、initialize/launch/attach、breakpoint、continue、step、threads、stack frames、scopes、variables、evaluate、outputを接続済み。DebugサイドバーはThreads／Stack Frames／Variables／Watchesを表示し、Task Outputとは分離している。**未完了：** 実アダプターでのlaunch/attach受入れ（macOS権限・対象プロセスが必要）、逆方向DAP request、変数の展開・frame選択を含む完全な対話型ツリーUI。
