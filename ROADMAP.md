...
| M13：Windows対応 | 🟡 Win32/D3D11・clipboard/dialog/IME/font/drop/ConPTY基盤 | OS非依存ABIをC/Nim双方のcontractsへ分離し、Win32 window/message loop、per-monitor v2 DPI、D3D11 device/swap chain/clear-present、基本入力callback、Unicode clipboard、Open/Save file dialog、IMM32 composition/result/candidate position、font discovery、Unicode file drop、ConPTYライフサイクルを追加。ConPTYのGUIターミナル接続、installer、実Windows検証は未完了 |
