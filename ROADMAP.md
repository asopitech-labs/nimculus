...
**実装済み追加項目：** Win32 virtual-keyを既存のAppKit key code契約へ変換し、Ctrl系標準ショートカットをCommand相当へ正規化して`shortcut callback`へ接続した。Zedのaccelerator境界を参考に、shortcut/control-keyを消費した場合は`WM_CHAR`を生成しない。
