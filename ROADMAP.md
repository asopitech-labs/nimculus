...
| M6：macOS プロジェクト・ワークスペース | 🟡 部分UI統合・検索/Git UI待ち | フォルダ選択からWorkspaceを開き、遅延列挙したルート直下ツリーをMetalテキスト表示する最小縦切りを追加。複数ルート、ファイル操作、`.gitignore`、fuzzy/ripgrep API、協調型検索ジョブ、FSEvents、Worktree状態分離、10万ファイル計測を実装。検索結果とGit UI統合が残る |
...
**完了条件：** [x] 起動時に全ファイル内容を読み込まず列挙、[x] FSEvents変更通知、[x] `.gitignore` 適用、[x] 複数ルート・ファイル操作・fuzzy/ripgrep検索API、[x] UIから分割実行できる協調型検索ジョブ、[x] ルート直下ファイルツリーの最小表示、[x] 10万ファイル列挙ベンチマーク、[x] Worktree rootごとのHEAD/branch状態分離API。検索結果UIとGit UI統合は残る。Git UI統合はM9で実施する。
