...
| M17：拡張システム | 🟡 macOS基盤実装・実機確認対象 | ZedのExtensionHost/WasmHostの責務分離を参考に、manifest読込・global/workspace discovery・language/grammar/LSP/theme/icon/snippet/task/command登録、WASM container/API version検証、external processの明示permission検証、Extensionsメニュー／Command Paletteからの安全なローカル拡張インストールを実装。公式Wasmtime CLIを直接argvで起動するmacOS WASM実行境界（extension rootのみpreopen、API/id/host capabilityを明示env、任意shellなし）に加え、Wasmtime C APIを動的解決するComponent Model/WASI Preview 2境界（fuel・resource limit・read-only preopen・明示export呼び出し）を実装。Componentは専用worker、idle polling、epoch interruptionによるキャンセルまでUIへ接続し、core moduleはCLIへフォールバックする。未知権限の拒否、実行・導入前の非同期macOS Allow/Denyシート、runtime status、Run WASM Extension導線も追加。Zedの全WIT world互換ではなく、versioned host capability contractを最小面として先に固定する |
