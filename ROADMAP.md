...
| M4：エディタバッファと編集コア | 🟡 実装済み・候補構造比較待ち | Piece Table、検証済み原子的編集、Undo/Redo、複数カーソル、位置変換、fuzzを実装。Rope/Gap Buffer/Piece Table比較を追加、Piece Tree/Hybrid比較が残る |
...
- [x] 選択、編集グループ
- [x] 100MB級ロードベンチマーク
- [x] Piece Table / Chunked Rope / Gap Bufferの比較ベンチマーク
- [ ] Piece Tree / Hybridの比較ベンチマーク
- [x] 決定的fuzz test
...
**完了条件：** [x] 起動時に全ファイル内容を読み込まず列挙、[x] 検索キャンセル、[x] FSEvents変更通知、[x] `.gitignore` 適用、[x] 複数ルート・ファイル操作・fuzzy/ripgrep検索API。10万ファイル級とWorktree分離の実機計測は残る。
...
**完了条件：** [x] 編集差分に応じた再解析、[x] 初期6文法のロード、[x] 構文ノードから表示・構造サービスを生成、[x] 文法追加手順を文書化。実エディタへの遅延ハイライト接続と大規模ファイル計測は残る。
