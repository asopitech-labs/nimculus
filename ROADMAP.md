...
**実装済み基盤：** `GitRepository` によるrepository検出、porcelain v1 NUL status解析（rename/copy/conflictを含む）、unstaged/staged diff、stage/unstage、commit、branch/HEAD、log、line blame、conflict path取得、終了コードを保持するGitJobと明示的cancel（`src/nimculus/git_service.nim`）。UI接続、inline diff/gutter、checkoutは残る。
