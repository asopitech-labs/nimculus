...
Win32、GPU backend（Direct3D または `wgpu-native`）、DPI、keyboard、IME、clipboard、drag and drop、file dialogs、font discovery、ConPTY、installer、fullscreen復元、minimize/maximize/restoreを実装する。
...
**実装済み追加項目：** Win32 virtual-keyを既存のAppKit key code契約へ変換し、Ctrl系標準ショートカットをCommand相当へ正規化して`shortcut callback`へ接続した。Zedのaccelerator境界を参考に、shortcut/control-keyを消費した場合は`WM_CHAR`を生成しない。Win32 mouse messageのclient座標とwheel messageのscreen座標を分け、両者をDPI logical pointへ変換する。
