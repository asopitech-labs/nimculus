...
**進捗：** 🟡 端末コアを実装済み・Nimculus UI統合待ち

**実装済み基盤：** ZedのPTYイベントループと端末画面の責務分離を参考に、macOS `forkpty`、non-blocking master、shell起動、入力書き込み、終了処理、ウィンドウサイズ通知、ANSI/VTの基本CSI処理、UTF-8 glyph、画面バッファ、scrollback、resizeを`src/nimculus/terminal.nim`へ実装。さらにZedのTaskSpec境界を参考に、working directory・引数・環境変数・終了コード・stdout/stderr統合・cancelを`src/nimculus/task_service.nim`へ実装し、Command Paletteの`run task <command>` / `cancel task`、idle polling、成功/失敗/キャンセルのstatus表示へ接続済み。PTY実行・画面更新・ANSI移動・UTF-8・resize・task実行・失敗・cancelを統合テストで検証済み。NimNUIのターミナルビュー、完全なVT parser、選択/clipboard、複数セッション、task/output panelは未接続。
