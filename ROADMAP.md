...
| M13：Windows対応 | 🟡 Win32/D3D11・clipboard/dialog/IME/font/drop/ConPTY/bundle基盤 | OS非依存ABIをC/Nim双方のcontractsへ分離し、Win32 window/message loop、per-monitor v2 DPI、D3D11 device/swap chain/clear-present、基本入力callback、Unicode clipboard、Open/Save file dialog、IMM32 composition/result/candidate position、font discovery、Unicode file drop、ConPTYライフサイクル、ZIP/Inno Setup bundle工程を追加。ConPTYのGUIターミナル接続、実Windowsinstaller生成・実操作検証は未完了 |
