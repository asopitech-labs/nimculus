...
| M9：macOS Git統合 | 🟡 自動E2E済み・実機確認対象 | 非同期status/diff、gutter、hunk stage/unstage、commit/log/blame/checkoutを実装。Git履歴はスクロール可能な専用サイドバーへ最新100件を表示し、選択したcommitのmetadata・統計・patchはexternal diffを起動せず出力パネルへ表示 |
