...
- [x] 検索、置換
- [x] ソフトラップをCore Text native描画へ接続し、Command Paletteの`toggle soft wrap`で切替。状態はsessionへ保存。カーソル、IME候補位置、クリックhit-testも同じ表示行マッピングを使用し、スクロール/インデントガイドもView状態とnative表示へ接続
