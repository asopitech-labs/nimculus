...
**進捗：** 🟡 設定コア・階層マージ・型検証・live reloadの基盤を実装済み・UI/キーバインド反映待ち

**実装済み基盤：** ZedのSettingsStore/KeymapFileを参考に、global/workspace/language設定の再帰マージ、JSON型検証、診断保持、ファイルmtimeによる再読み込み、theme color、icon/theme設定値、keymap配列、terminal shell、LSP commandの設定取得を`src/nimculus/settings.nim`へ実装。macOS起動時にApplication Supportのglobal settingsとworkspace `.nimculus/settings.json`を読み込み、idle時の変更検知とterminal shell/LSP commandへの反映を接続済み。設定UI、標準Command/Optionキーマップの設定ファイル反映、テーマ描画への完全反映、icon theme、schema公開は残る。
