...
| M5：macOS 最小実用エディタ | 🟡 自動E2E済み・実機確認対象 | 編集サービス、plain-text fallbackを含む動的文書表示、構文色、macOSメニュー/IME/Finder接続、Application Supportへのsession復元・crash recovery、`Cmd+,`の設定パネル導線を実装。Find/Replace/Go to LineとCommand PaletteはZed同様の非モーダルなエディタ内バーとして実装。二paneの対話操作は受け入れ記録へ集約 |
...
| M10：macOSターミナル・タスク | 🟡 自動検証済み・E2E対象 | PTY、VT/ANSI、複数session、task/cancel/problem matcher/output panelを実装。`Close Terminal`は検証済みの直接PTY子プロセスだけを終了し、残りのsessionを選択する。連続利用はE2Eで確認 |
