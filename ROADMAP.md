...
| M3：macOS テキスト描画と IME | 🟡 実装済み・GUI実機検証待ち | Core Text、動的Metal文字テクスチャ、Tree-sitter構文色、IME、候補位置、clipboardを実装。日本語IME、カーソル/選択、Retina文字表示の実機確認が残る |
...
| M6：macOS プロジェクト・ワークスペース | 🟡 API/ベンチマーク実装済み・サービス統合待ち | 遅延ファイルツリー、複数ルート、ファイル操作、`.gitignore`、fuzzy/ripgrep API、FSEvents、Worktree状態分離、10万ファイル計測を実装。非同期検索サービスとGit UI統合が残る |
| M7：Tree-sitter | 🟡 実装済み・GUI実機検証待ち | Nim/Rust/TypeScript/Python/JSON/MarkdownのFFI、増分解析、構文状態、可視範囲ハイライト、RGBA Metalテクスチャ接続を実装。大規模ファイル計測とGUI実機確認が残る |
...
**完了条件：** [x] 起動時に全ファイル内容を読み込まず列挙、[x] FSEvents変更通知、[x] `.gitignore` 適用、[x] 複数ルート・ファイル操作・fuzzy/ripgrep検索API、[x] 10万ファイル列挙ベンチマーク、[x] Worktree rootごとのHEAD/branch状態分離API。検索APIのキャンセル契約は実装済みだが、現在の同期実装をUI非ブロッキングの非同期サービスへ接続する作業が残る。Git UI統合はM9で実施する。
...
**完了条件：** [x] 編集差分に応じた再解析、[x] 初期6文法のロード、[x] 構文ノードから表示・構造サービスを生成、[x] 実エディタの可視範囲ハイライトとRGBA Metalテクスチャへ接続、[x] 文法追加手順を文書化。大規模ファイル計測とGUI実機での色表示確認が残る。
