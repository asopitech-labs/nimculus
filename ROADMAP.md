...
**進捗：** 🟡 プロトコル・stdio・要求アダプタ基盤実装済み・機能統合待ち
...
**実装済み基盤：** UTF-8 byte-accurate `Content-Length` framing、partial/multiple frame decoder、JSON-RPC request construction、method generationによるstale response破棄、cancel/timeout状態管理、stdio processのsend/read、EOF/exit状態、stop/restart、initialize handshake、document同期、diagnostics parsing/cache、UTF-16→UTF-8 byte診断範囲変換、completion/hover/navigation/rename/formatting/code action/signature help/semantic tokens/inlay hint要求生成、locations/text edits/completion/hover/symbols response parser、syntax highlightと分離したdiagnostic span ABI、severity別のCore Text/Metal overlay下線表示（`src/nimculus/lsp.nim`、`src/nimculus/editor_diagnostics.nim`、`src/nimnui/platform/macos/macos_platform.m`）。
