...
| M5：macOS 最小実用エディタ | 🟡 自動E2E済み・実機確認対象 | 編集サービス、plain-text fallbackを含む動的文書表示、構文色、macOSメニュー/IME/Finder接続、Application Supportへのsession復元・crash recovery、`Cmd+,`の設定パネル導線を実装。Find/Replace/Go to LineとCommand PaletteはZed同様の非モーダルなエディタ内バーとして実装。Metalの上原点座標とAppKit child frameの下原点座標を境界で変換し、タブ・行番号・サイドバー・overlayを本文と同じpaneへ配置する。二paneの対話操作は受け入れ記録へ集約 |
