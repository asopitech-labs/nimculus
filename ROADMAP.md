...
**実装済み基盤：** ZedのPTYイベントループと端末画面の責務分離を参考に、macOS `forkpty`、non-blocking master、shell起動、入力書き込み、終了処理、ウィンドウサイズ通知、ANSI/VTの基本CSI処理、UTF-8 glyph、画面バッファ、scrollback、resizeを`src/nimculus/terminal.nim`へ実装し、PTY実行・画面更新・ANSI移動・UTF-8・resizeを統合テストで検証済み。NimNUIのターミナルビュー、完全なVT parser、選択/clipboard、複数セッション、タスク/output panelは未接続。
