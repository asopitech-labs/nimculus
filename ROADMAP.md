...
**進捗：** 🟡 端末コア・macOS最小UI統合済み・複数セッション/完全UI待ち

**実装済み基盤：** ZedのPTYイベントループと端末画面の責務分離を参考に、macOS `forkpty`、non-blocking master、shell起動、入力書き込み、終了処理、ウィンドウサイズ通知、CSI移動/消去、OSCメタデータ、DEC alternate screen、cursor visibility、UTF-8 glyph、画面バッファ、scrollback、resizeを`src/nimculus/terminal.nim`へ実装。TerminalScreenは可視行とscrollbackを含むcell selectionを保持し、NimNUI clipboardへコピーできる。さらにZedのTaskSpec境界を参考に、working directory・引数・環境変数・終了コード・stdout/stderr統合・cancelを`src/nimculus/task_service.nim`へ実装し、Command Paletteの`run task <command>` / `cancel task`、idle polling、成功/失敗/キャンセルのstatus表示へ接続済み。macOS Metal editor上へ非編集Terminal overlayを追加し、`toggle terminal` / `new terminal`、PTY出力表示、selection/copy/paste、Enter/Tab/Backspace/矢印/Ctrl-C入力、resize追従を接続済み。PTY実行・画面更新・ANSI/OSC・alternate screen・selection・UTF-8・resize・task実行・失敗・cancelを統合テストで検証済み。完全なVT parser、複数セッション、task/output panelは未接続。
