...
**update基盤：** Zedのrelease判定とインストール責務の分離を参考に、HTTPS artifactのみを受理するmanifest parser、semver比較、SHA-256 artifact検証、Command Paletteの`check for updates`（`NIMCULUS_UPDATE_MANIFEST`指定時）を実装する。署名検証と終了時インストールは、manifest取得後の次段階として未完了のまま残す。
