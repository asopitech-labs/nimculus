...
**計測導線：** `tests/bench_m20.nim` を `nimble benchmark` の先頭から実行し、editor load/edit、Unicode text position、Tree-sitter visible syntax、terminal parser、workspace enumerationをTSVで記録する。`NIMCULUS_BENCH_EDITOR_BYTES` と `NIMCULUS_BENCH_FILES` で100MB級・10万ファイル級へ拡張できる。cold start、idle memory、input/frame/layout、file watcher、LSP、remote、allocationの実機計測は未完了として残す。
