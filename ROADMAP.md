...
| M17：拡張システム | 🟡 macOS基盤実装・実機確認対象 | ZedのExtensionHost/WasmHostの責務分離を参考に、manifest読込・global/workspace discovery・language/grammar/LSP/theme/icon/snippet/task/command登録、WASM container/API version検証、external processの明示permission検証、Extensionsメニュー／Command Paletteからの安全なローカル拡張インストールを実装。公式Wasmtime CLIを直接argvで起動するmacOS WASM実行境界（extension rootのみpreopen、API/idを明示env、任意shellなし）に加え、Wasmtime C APIを動的解決するComponent Model/WASI Preview 2境界（fuel・resource limit・read-only preopen・明示export呼び出し）を実装。Componentは専用worker、idle polling、epoch interruptionによるキャンセルまでUIへ接続し、core moduleはCLIへフォールバックする。runtime status、Run WASM Extension導線も追加。Zed互換のWIT host API、権限UI、配布カタログは未完了 |
...
今回の継続作業では、ZedのWASM hostが実行前にAPI versionを検証し、manifest登録とホスト実行を分離している点を反映した。Extensionsメニュー／Command Paletteの`Install Extension…`からローカルフォルダを選択し、`extension.json`、安全なID、symlink、WASM containerを検証して`~/.nimculus/extensions/<id>`へ原子的にコピーし、失敗時は一時領域を掃除する。さらに公式Wasmtime CLIを解決し、core moduleとComponent Modelの両方を認識し、extension rootだけを`/extension`へpreopenする直接argv実行計画を追加した。`extensions runtime`でruntime状態を確認し、`extensions run`で単一WASM拡張をTask境界から実行できる。加えて、Wasmtime公式C APIのComponent linker、WASI Preview 2、fuel、store limiter、export lookup/call、epoch interruptionのABIを確認し、Homebrewへリンクせず`dlopen`で同一アーキテクチャのライブラリだけを任意利用するmacOS境界を追加した。Componentは専用pthread workerから実行し、macOS idleで状態をpollし、キャンセル時はepochを進めてUIスレッドをブロックしない。core moduleは既存CLIへフォールバックする。Zed互換のWIT host API、権限UI、配布カタログは未完了として残す。
