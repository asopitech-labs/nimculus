...
| M6：macOS プロジェクト・ワークスペース | 🟡 API/ベンチマーク実装済み・UI統合待ち | 遅延ファイルツリー、複数ルート、ファイル操作、`.gitignore`、fuzzy/ripgrep API、協調型非同期検索ジョブ、FSEvents、Worktree状態分離、10万ファイル計測を実装。ファイルツリー/検索のアプリUI統合とGit UI統合が残る |
...
**完了条件：** [x] 起動時に全ファイル内容を読み込まず列挙、[x] FSEvents変更通知、[x] `.gitignore` 適用、[x] 複数ルート・ファイル操作・fuzzy/ripgrep検索API、[x] UIから分割実行できる協調型検索ジョブ、[x] 10万ファイル列挙ベンチマーク、[x] Worktree rootごとのHEAD/branch状態分離API。ファイルツリー/検索のアプリUI統合とGit UI統合は残る。Git UI統合はM9で実施する。
