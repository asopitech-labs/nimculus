...
| M9：macOS Git統合 | 🟡 自動E2E済み・実機確認対象 | 非同期status/diff、gutter、hunk stage/unstage、commit/log/blame/checkoutを実装。`git branches`はlocal branch一覧を表示し、`git switch <branch>`はGitの安全確認を維持して切替・clean tab再読込を行う。Git履歴はスクロール可能な専用サイドバーへ最新100件を表示し、選択したcommitのmetadata・統計・patchはexternal diffを起動せず出力パネルへ表示 |
