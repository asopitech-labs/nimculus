...
| M3：macOS テキスト描画と IME | 🟡 自動E2E済み・実機確認対象 | Core Text、glyph atlas、動的Metal文字描画、Tree-sitter構文色、marked text表示、IME、候補位置、clipboardを実装。Metalの上原点とCore Graphicsの下原点を変換したtext viewport clipを追加し、右・下のpane chromeへCore Text文字が漏れないよう固定。日本語IMEの対話確認は受け入れ記録へ集約 |
