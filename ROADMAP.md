...
| M13：Windows対応 | 🟡 Win32/D3D11・clipboard/dialog/IME/font/drop縦切り | OS非依存ABIをC/Nim双方のcontractsへ分離し、Win32 window/message loop、per-monitor v2 DPI、D3D11 device/swap chain/clear-present、基本入力callback、Unicode clipboard、Open/Save file dialog、IMM32 composition/result/candidate position、font discovery、Unicode file dropを`src/nimnui/platform/windows`へ追加。ConPTY、installer、実Windows検証は未完了 |
