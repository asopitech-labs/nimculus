...
**update基盤：** Zedのrelease判定とダウンロード／インストール責務の分離を参考に、HTTPS artifactのみを受理するmanifest parser、semver比較、HTTPS download、SHA-256 artifact検証、Command Paletteの`check for updates`（`NIMCULUS_UPDATE_MANIFEST`指定時）、macOS `.app`に対する`codesign --verify`／`spctl --assess`検証APIを実装する。署名済みアプリの終了時インストールとnotarization済み配布物による実機検証は未完了のまま残す。
