...
**GPU描画追加：** 既存の`NativePaintCommand` ABIをWindows backendへ接続し、D3D11 dynamic vertex buffer・runtime shader・per-command scissorでopaque rectangle-like primitiveを描画する。文字/image primitiveはglyph/sprite resource実装後の残課題とする。
