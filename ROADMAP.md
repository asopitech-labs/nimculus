...
| M2：NimNUI 基礎 UI システム | 🟡 自動E2E済み・実機確認対象 | UIツリー、レイアウト、状態、イベント、PaintList、macOS入力を実装。縮小時は論理dockの実効幅に合わせてdock全体を退避し、空の右領域や枠外sidebarを残さずエディタ幅を回復する。自動E2Eで統合基準を確認済み。個別GUI操作は受け入れ記録へ集約 |
