...
| M13：Windows対応 | 🟡 契約分離・非macOS compile gate | OS非依存のmetrics/input/paint/annotation ABIを`src/nimnui/platform/contracts.nim`へ分離し、macOS以外は明示的headless backendを選択。Linux条件コンパイルを通過。Win32、GPU、DPI、IME、clipboard、ConPTY、installer本体は未着手 |
...
**先行基盤：** `nimnui/nimnui.nim`はmacOS時だけCocoa/Metal backendを選択し、それ以外では`platform/headless/platform.nim`を選択する。headless backendは共有契約の安全なno-opであり、Windows機能の完了を意味しない。`nim c --compileOnly --os:linux`でNimculusアプリ層がCocoa依存なしにコンパイルできることを確認する。
