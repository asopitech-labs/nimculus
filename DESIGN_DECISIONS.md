...
## UI-117: ドックのトグルは、そのドックの位置と同じ側に出す
...
**端末のトグルが Zed には左に無い。** 区切りの位置も 1 つずれている。

### Zed の規則

`crates/workspace/src/workspace.rs:1757-1759`:
...
**トグルが出る側は、そのドックの位置で決まる。** 左ドックのパネルは左、
右ドックと下ドックのパネルは右。Zed の端末は下ドックなので、
トグルは**右端**にある。

区切りも `dock.rs:1392-1402` で規則化されている:
...
左ドックは**ボタンの後ろ**に区切り、下・右ドックは**ボタンの前**に区切り。
つまり区切りは常に「ドックのトグル群と、それ以外のステータス項目」の境界にある。

さらに右ドックのボタンは `dock.rs:1447` で `buttons.reverse()` される。
...
**UI-097 の配置部分は本項で置き換える。** ただし UI-097 の他の内容
（`commandPalette:toggle terminal` の維持、28pt 行 / 24pt ヒット領域、
セマンティックな境界色）はそのまま有効。

### 移植できるか

できる。`src/nimculus/workspace_ui.nim:63,78` に `leftDock` / `bottomDock` があり、
`DockState.side` が `dockLeft` / `dockBottom` を持っている。**位置の情報は既にある。**
使っていないだけ。

### やること

1. トグルを出す側を `DockState.side` から決める
2. 区切りの位置も Zed の規則（左＝後ろ、下と右＝前）に従わせる
3. 端末のトグルは下ドックなので**右クラスタへ移す**
...
- 契約テスト: 下ドックのパネルのトグルが右クラスタに出ること、
  左ドックのそれが左クラスタに出ること、区切りの位置
- キャプチャ: フッタ左端が `[パネル] [✨] │ [🔍] [✓]` になること
