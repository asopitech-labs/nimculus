...
## UI-122: タイトルバーの高さが 2pt 低い / 罫線の色がテーマ値でない

キャプチャの帯比較で、タブバー帯の**行 68–69 が 99.9%**（ほぼ全幅）違っていた。
罫線の位置がずれている。

### 実測（2026-08-10、テスト VM、device px。2x なので pt は半分）

| row | Nimculus | Zed |
| --- | --- | --- |
| 62–63 | `#dcddde` タイトルバー | `#dcdcdd` タイトルバー |
| **64–65** | **`#cfd1d2` 罫線** | `#dcdcdd` タイトルバー |
| 66–67 | `#ececed` タブバー | `#dcdcdd` タイトルバー |
| **68–69** | `#ececed` タブバー | **`#c9c9ca` 罫線** |
| 70– | `#ececed` | `#ebebec` タブバー |

**Zed のタイトルバーは 34pt、Nimculus は 32pt。** 2pt 低い。

Nimculus の内容全体を 4 device px 下げると、rows 60–400 の
`>32` が 6.23% → **5.12%** に改善する。ずれは全体に及んでいる。

### (1) 高さ: Zed は rem から計算する
...
既定 rem = 16px なので `max(28, 34)` = **34px**。実測と一致する。

Nimculus は `macos_platform.m:443` の
`static const CGFloat NimculusTitlebarHeight = 35.0;` を持つが、
**実際に塗られているのは 32pt**。定数と描画結果が一致していない。
どこで 32 になっているかを調べる必要がある。

**rem に連動しないのも差。** Zed は rem が大きければ 34 を超える。

### (2) 罫線の色: 観測値を焼き込んでいる

| | 値 |
| --- | --- |
| One Light のテーマ JSON（`one.json:429`）| `#c9c9ca` |
| VM 内の Zed が塗る値 | **`#c9c9ca`** |
| Nimculus（`settings.nim:553`）| `#cfd1d2` |

`UI_PARITY_HANDOFF.md` の罠 10 が「`border` は JSON `#c9c9ca` に対し
実描画 `#cfd1d2`」と記録し、その**観測値をテーマ表に入れた**。
UI-112 と同じ誤りで、対象が `border` である点だけが違う。

**UI-112 との違い**: UI-112 は Metal の最終塗りに焼き込んだ補正で、
AppKit と Metal の色管理差が原因だった（UI-111 の下流）。
本項は**テーマ表の値そのもの**が観測値になっている。
`settings.nim` の値を JSON の値に直せば済む。UI-111 に依存しない。

同じ疑いのある値（罠 10 に列挙されているもの）:

| 役割 | JSON | settings.nim |
| --- | --- | --- |
| `border` | `#c9c9ca` | `#cfd1d2` |
| `border.variant` | `#dfdfe0` | `#dfe0e1` |

**両方とも実測で確かめてから直すこと。**

### やること

1. タイトルバーの高さを Zed の式（`max(1.75 * rem, 34)`）にする。
   現在 32pt になっている原因を先に特定する
2. `border` と `border.variant` の値をテーマ JSON の値に直す。
   ただし**先に VM で Zed の実描画を測り**、JSON 値と一致することを確認する

### 却下案

**(a) タイトルバーを 34 の定数にする。** Zed は rem に連動する。
定数にすると、フォントサイズを上げたときに Zed と離れる。却下。

**(b) 罫線の色は参照ディスプレイで合っているので変えない。**
UI-112 の訂正と同じ理由で却下。テーマ値を塗るのが Zed の実装であり、
観測値は環境依存。

### テスト観点

- タイトルバーの罫線が 68–69 device px に来ること
- rem（フォントサイズ）を変えたとき高さが追従すること
- 罫線の色が `#c9c9ca` / `#dfdfe0` になること
- 帯比較: タブバー帯の `>32` が下がること（現状 11.53%）
