...
### 実測（2026-08-10、同じ git リポジトリ、同じファイル）

| | Zed | Nimculus |
| --- | --- | --- |
| **色** | `#d3b020`（テーマの `modified`）| **`#79b0f6`**（青。テーマに無い）|
| **位置** | cols **0–11**（左端から）| cols **20–22** |
| **幅** | **12 device px**（6pt）| **3 device px**（1.5pt）|
| **範囲** | rows 320–367（**1 行**）| rows 234–369（**約 3 行**）|

**4 つとも違う。**

### Zed の幅は行高から決まる

`crates/editor/src/element.rs:5309-5311`:
...
行高 24pt → `floor(6.6)` = **6pt**。実測の 12 device px（2x）と一致。

**固定値ではなく行高の 0.275 倍。** フォントサイズを変えると追従する。

位置は `diff_hunk_bounds`（`:5313`）で、
`highlight_size = size(gutter_strip_width, end_y - start_y)`（`:5330`）。
**ガターの左端から。**

### 色はテーマから来る

Zed が塗った `#d3b020` は `settings.nim:539` の `modified` と同じ値。
**Nimculus はテーマに `modified` を持っているのに、
ガターでは `#79b0f6` を使っている。**

UI-112 / UI-123 / UI-124 / UI-128 と同じ構図
（テーマの値があるのに別の色を塗っている）。**5 件目。**

### 範囲が 3 行なのは別の問題

Zed は 1 行だけ、Nimculus は約 3 行に引いている。
**hunk の判定そのものが違う**可能性がある。
色と幅を直しても、この差は残る。

**先に判定を確認すること。** 同じリポジトリの同じファイルで、
変更されている行が何行あるかを `git diff` で確かめ、
どちらが正しいかを決める。

### やること（順序）

1. **hunk の判定を確かめる**（`git diff` の実際の出力と突き合わせ）
2. 色をテーマの `modified` にする
3. 幅を `floor(0.275 * lineHeight)` にする
4. 位置をガターの左端にする

**1 を先に。** 判定が違うまま見た目を合わせると、
「色は合っているが行が違う」状態になる。

### 却下案

**(a) 幅を 6pt 固定にする。** Zed は行高から計算する。
フォントサイズを変えたときに離れる。UI-122 の却下案 (a) と同じ。却下。

## UI-133: フォルダを開いた条件で出た差

キャプチャの条件が**単一ファイルだけ**だったので、
git リポジトリのフォルダを開いた比較を足した
（`testCaptureWorkspace`、`nimculus-workspace.png` / `zed-workspace.png`）。

**その条件でしか描かれないものが 4 つ出た。**

### 実測（2026-08-10、テスト VM、同じ git リポジトリを開いた状態）

```
overall identical 93.01%   >32 5.41%   mean 4.67
  titlebar   86.02%   >32  6.84%
  tabbar     88.05%   >32  7.09%
  toolbar    86.21%   >32 11.03%
  editor     96.56%   >32  3.10%
```

**エディタ本文は 96.56%。** 単一ファイル条件の 92.35% より高い。

| | Zed | Nimculus |
| --- | --- | --- |
| タイトルバーの名前 | **`workspace-capture-repo`**（フォルダ名）| `workspace_target.txt`（ファイル名）|
| git 項目 | **2 つ**（`⇥ main` と `⑂ ui-132-workspace`）| 1 つ（ブランチのみ）|
| タブバー右端 | 何も無い | **`⌄` `+`** |
| ガター 1 行目 | マーカー無し | **青いバー** |

### (1) 名前は「ワークツリーのルート名」

`title_bar.rs:241-245`:

```rust
project_name = worktree.read(cx).root_name()
    .file_name()
    .map(|name| SharedString::from(name.to_string()));
```

**開いているワークツリーのルートの名前。**
単一ファイルを開いたときはそのファイル名になり、
フォルダを開いたときはフォルダ名になる。

UI-132 で「開いているものの名前」と書いて実装したが、
**Nimculus はフォルダを開いてもファイル名を出す**。
ワークツリーのルート名ではなく、アクティブなファイル名を使っている。
...
**リンクされたワークツリーの名前**（`git worktree` の副ワークツリー）を出し、
無ければ `"main"`。これがブランチ名とは**別の項目**として並ぶ。

`:248-259` が `main_worktree_abs_path` から短縮名を求める。
通常のリポジトリでは副ワークツリーが無いので `main` が出る。

**Nimculus はこの項目を持たない。**

### (3)(4) 未確認

- タブバー右端の `⌄` `+`: Zed に無いものを出している可能性。
  Zed のタブバーに該当するコントロールがあるか未確認
- ガター 1 行目の青いバー: git hunk の表示。
  同じリポジトリで Zed が出していないので、**判定が違う**可能性。
  1 行目はコミット済みのはずで、変更マーカーが出るのはおかしい

**(3)(4) は次に調べる。**

### 意味

**条件を広げたら、それまで見えなかった差が 4 つ出た。**
単一ファイルの比較では、この 4 つはどれも描かれない。

UI-132 は単一ファイルの条件で「正しく移植した」と判定したが、
フォルダの条件では (1) が誤っている。
**1 つの条件で確認した移植は、その条件でしか確認されていない。**
...
1 と 2 を移植した。**ファイル名が出て `No Git branch` が消えた。**

| | ファイル名の x 範囲 | 幅 |
| --- | --- | --- |
| Zed | 386–737 | 351 |
| Nimculus | 154–521 | 367 |

**幅はほぼ同じ**（差 16px ≒ 4.5%、フォントの送り幅の差）。
**位置が 232px ずれている。** Zed は `Restricted Mode` バッジのぶん右にある。

タイトルバー帯の `identical` は 87.55% → **87.26%** と**下がった**。

**正しく移植した結果、指標が下がった。**
相手側にこちらが持たない機能（ワークスペースの信頼）があり、
その表示がレイアウトを押しているため。

### この帯の残差は Restricted Mode

以後、タイトルバー帯の `identical` はワークスペースの信頼機能を
移植するまで上がらない。**フォントのフォールバック（§5.1）と同じ性質**の、
機能差に由来する受容すべき残差。

**指標が下がったことを回帰と読まないこと。** 移植の正しさは
「ファイル名が出ている」「`No Git branch` が消えた」で判定する。
...
| 帯 | UI-130 前 | UI-130 後 |
| --- | --- | --- |
| titlebar | 87.55% | 87.55% |
| tabbar | 89.29% | **93.35%** |
| toolbar | 71.11% | **94.10%** |
| editor | 69.50% | **92.35%** |
| **statusbar** | 80.69% | **82.05%** ← 最低 |

**ステータスバーが最低になった。**

### 実測（2026-08-10、テスト VM、フッタ左端）

構造は一致している（UI-117 の効果）。`[▯] [✨] │ [🔍] [✓]`。

| | アイコンの x 幅 | 墨の総量 |
| --- | --- | --- |
| Zed | 9 / 23 / 21 / 19 / 13 | **670** |
| Nimculus | 14 / 31 / 21 / 26 / 23 | **866** |

縦は両方 y 14–51（38 device px = 19pt）で一致。
**横だけ広く、墨が 29% 多い。**

### Zed のアイコンの大きさ

`crates/ui/src/components/icon.rs:74`:

```rust
IconSize::Small => rems_from_px(14.),
```

ステータスバーの項目はすべて `IconSize::Small` = **14px**
（`search_status_button.rs:32`、`items.rs:38`、`dock.rs:1359` など）。

パディングは `DynamicSpacing::Base02`（`:91`）。

### Nimculus の現状

SF Symbols を使っており、点サイズの指定を確認する必要がある。
**縦は一致していて横が広い**ので、単純な点サイズの差ではなく、
字形（SF Symbols と Zed の独自アイコン）の差の可能性がある。

**測ってから判断する。** 点サイズを変えれば縦も動くので、
縦が一致している現状から、点サイズだけの問題ではないと分かる。

### 未確定

- Zed のアイコンは独自の SVG（`assets/icons/`）。
  SF Symbols とは字形が違う。**同じ絵にはならない**
- どこまで寄せるかは、字形の差を受容するかどうかの判断
- フォントのフォールバック（§5.1）と同じ性質の差になる可能性がある

**次に着手するなら、まず「SF Symbols を使い続けるか」を決める。**
Zed のアイコンを同梱するなら UI-111（AppKit chrome）に触れる
（`NSButton` の `NSImage` に SVG を描く先が無い）。
...
### Zed の入力源は 6 つ、こちらは 1 つ

`activity_indicator.rs:337-540` の `content_to_render` は
**優先順位つきの連鎖**で、上から順に見て最初に該当したものを返す:

| # | 源 | Nimculus |
| --- | --- | --- |
| 1 | direnv のエラー | 無し |
| 2 | **LSP の `$/progress`** | **UI-126 で揃った** |
| 3 | DAP セッションの起動待ち | 無し（DAP はあるが起動待ちの状態を持たない）|
| 4 | 長時間の git ジョブ（`GIT_OPERATION_DELAY` 超過）| ジョブの追跡が無い |
| 5 | 長時間の fs ジョブ | 無し |
| 6 | LSP バイナリの取得状況 | 無し（サーバを自分で取得しない）|

**6 つのうち 1 つだけが揃った状態。**

### 表示の仕様（`:649-700`）

- `Button` にメッセージ、アイコンは `LoadingSpinner` か `Icon`
- **50 文字で切って `…`**（`MAX_MESSAGE_LEN = 50`、`truncate_and_trailoff`）
- 切ったときはツールチップに全文
- `on_click` があればボタン、無ければポップオーバーメニュー
- 表示するものが無ければ**空の `h_flex` を返す**（`:655-657`）

`$/progress` のときの文言（`:361-375`）:
...
**タイトルが無ければトークン名を出す。** 最初の 1 件だけ表示し、
残りは `+ N more`。アイコンは `LoadingSpinner`。

### 判断: 1 つでも作る

**作る。** 理由:

1. **Zed の構造は優先順位つきの連鎖**であり、源が 1 つでも成立する。
   源を足すのは連鎖に分岐を足すこと。**構造を変えずに増やせる**
2. 表示が無いと、UI-126 で受信した進捗が**動いていることを確認できない**。
   保持しているだけでは、実際の言語サーバで動くかどうかが分からない
3. 残り 5 つの源は**それぞれ別の機能の移植**（direnv、DAP の状態、
   ジョブ追跡、サーバの自動取得）。1 つの表示のために
   5 つの機能を先に作るのは順序が逆

### 却下案

**(a) 6 つ揃うまで作らない。** 5 つはそれぞれ独立した機能の移植で、
いつ揃うか分からない。その間 `$/progress` は検証できないまま残る。却下。

**(b) 連鎖を作らず、LSP 進捗を直接表示する。** 源が増えたときに
書き換えることになる。Zed の `content_to_render` は
**源を足す前提の形**なので、最初からその形にする。却下。

**(c) 未実装の源を「無し」として連鎖に置く。** 分岐だけ置いても
中身が無いので、読む人には「実装したが動かない」と見える。
**該当する源だけを連鎖に置き、無いものは書かない。**
何が無いかは本項に記録する。採用。

### やること

1. `content_to_render` 相当の連鎖を作る（源は `$/progress` の 1 つ）
2. 文言を Zed のとおりに組む（タイトルまたはトークン名、
   `(N%)`、`: メッセージ`、`+ N more`）
3. 50 文字で切り、切ったらツールチップに全文
4. アイコンは `LoadingSpinner`
5. 表示するものが無ければ何も出さない

### 置き場所

Zed では**ステータスバーの左**（`zed.rs:641` の `add_left_item`）。
UI-114 で左側の 7 項目を調べたときの 5 番目。
UI-117 の規則（ドックのトグルは位置で決まる）には関わらない
（ステータス項目であってドックのトグルではない）。
...
本文の残差を調べていて見つけた。**罠 1 が誤りだった。**

### 実測（2026-08-10、テスト VM、同じ文書の同じ見出し行）

| | 見出しの色 |
| --- | --- |
| **Zed** | **`#d3604f`**（1742 px）|
| Nimculus | `#1b1c1f`（740 px、ほぼ黒）|

one.json の light テーマ:

```
syntax.title.color = #d3604fff
```

**Zed はテーマの `syntax.title` をそのまま塗っている。**

### 罠 1 の記録

`UI_PARITY_HANDOFF.md` の罠 1:

> テーマ JSON を読んで色を決めた → Markdown 見出しに `syntax.title`
> （赤 `#d3604f`）を適用したが、Zed の実描画は `#5d6165`
> （通常のグレー＋太字）だった。**必ず実ピクセルを見る**。

**この観測が誤っていた。** どういう条件で `#5d6165` に見えたのかは不明。
考えられるのは、その文書が Markdown と認識されていなかった、
Zed の Markdown 文法が読み込まれていなかった、
別の見出しレベルを見ていた、など。

いずれにせよ**現在の VM での実測は `#d3604f`** であり、
テーマ値と一致する。UI-112 / UI-123 と同じ構図
（参照ディスプレイでの観測が実装をテーマから遠ざけた）。

### 未解明: Zed に先頭の空行がある

同じキャプチャで、**Zed は 1 行目が空行、2 行目が見出し**。
Nimculus は 1 行目が見出し。

ホストの `DEVELOPMENT_GUIDELINES.md` は `#` で始まる（先頭に空行はない）。
ゲストへは `git ls-files` の tar で配るので内容は同じはず。

ハーネスの変更（カーソル位置を揃えた）より**前のキャプチャでも同じ**なので、
今回の変更が原因ではない。**どちらのエディタが正しいかを確かめる必要がある。**

この 1 行ずれのせいで、本文の色（Nimculus `#1b1c1f` /
Zed `#242529`）が同じ内容の行どうしで比べられていない。
**先にこれを解決してから本文の色を判断する。**

### やること

1. ゲスト内のファイルの先頭バイトを確認し、どちらが正しいかを決める
2. 見出しの色を `syntax.title` にする
3. 罠 1 を訂正する
4. 1 行ずれが解消してから、本文の色を測り直す

## UI-122: 罫線の色が観測値になっていた（高さの指摘は私の測定ミス）

### 訂正（2026-08-10）: 「タイトルバーが 2pt 低い」は誤り

本項に「Zed のタイトルバーは 34pt、Nimculus は 32pt」と書いたが、**誤り**。

原因は**私の比較方法**。ウィンドウの高さが違う
（Nimculus 1360 / Zed 1356 device px）のに、**下端で揃えて**比較していた。

```python
nb = n[n.shape[0]-h:]   # 下端揃え
zb = z[z.shape[0]-h:]
```

これで上部を測ると 4px ずれる。**上端で揃えると罫線は両方 rows 68–69** で
一致する。高さは合っていた。
...
**帯によって揃える端が違う。** 上部の帯は上端、下部の帯（ステータスバー）は
下端で揃える。ウィンドウの高さが一致しない限り、片方の端でしか合わない。

（ステータスバーの測定は下端揃えなので、UI-113 の数値は有効。）

### 実測して直したもの: 罫線の色

| 役割 | テーマ JSON | VM 内の Zed の実描画 | Nimculus（旧）|
| --- | --- | --- | --- |
| `border` | `#c9c9ca` | **`#c9c9ca`**（rows 68–69）| `#cfd1d2` |
| `border.variant` | `#dfdfe0` | **`#dfdfe0`**（rows 222–223）| `#dfe0e1` |

`UI_PARITY_HANDOFF.md` の罠 10 が記録した実描画値は**参照ディスプレイでの
観測値**であって、Zed が塗る値ではなかった。UI-112 と同じ誤り。

**UI-112 との違い**: UI-112 は Metal の最終塗りに焼き込んだ補正で、
AppKit と Metal の色管理差が原因（UI-111 の下流）。本項は
テーマ表の値そのものなので、`settings.nim` を直せば済んだ。

### ついでに入れたもの: 高さを rem に連動させる

高さ自体は合っていたが、Nimculus は定数、Zed は
`max(1.75 * rem_size, 34)`（`constants.rs:17-19`）で**rem に連動する**。
既定では同じ 34pt だが、フォントサイズを変えると離れる。
式のほうを移植した。

### まだ残っている差

タブバー帯の `>32` は 6.11%。罫線の色差は解消したので、
残りは**アクティブタブの面**の差。次に調べる。
...
### 実装後に判明したこと（2026-08-09）

#### 「文字の幅」に 3 つの異なる量が使い分けられている

| 用途 | Zed の関数 | 実体 |
| --- | --- | --- |
| `padding` | `em_width`（`text_system.rs:226`）| `typographic_bounds('m').size.width` = **'m' の字面の幅** |
| `min_column` | `column_pixels`（`editor.rs:11736`）| **空白を N 個組版した幅** |
| （別用途） | `em_advance`（`text_system.rs:233`）| `advance('m').width` = **'m' の送り幅** |

**`em_width` は送り幅ではない。** ドキュメンテーションコメントが付いているのは
`em_advance` のほうだけで、`em_width` は名前から送り幅と読める。読み違えた。

実測（15pt、2x 画素）:

| | 字面幅 | 送り幅 | 比 |
| --- | --- | --- | --- |
| Nimculus（システム等幅）| 7.661pt | 9.272pt | 0.826 |
| Zed（Lilex）| 7.36pt | 9.0pt | 0.818 |

送り幅を使っていたときの実効 em は 18.57px、Zed の 14.71px に対し 1.26 倍。
字面幅にして 15.32px、1.04 倍になった。残差はフォントの違いで、
`UI_PARITY_HANDOFF.md` §5.1 の受容済みの差。

**絶対値では「近づいた」を判定できない**（フォントが違う）。
同じフォント内での**字面／送りの比**が Zed と一致することで、
同じ量を取っていることを確認した。

#### 回帰: `padding=0` で本文が消えた

行内 blame を本文の行に合成する形にしたため、`padding=0` で
開始位置が本文と重なり、**本文を上書きしていた**。
既定は `padding=7` なので通常の使用では露見しない。

Zed は本文と blame を**別の要素**として描く（`element.rs:2085-2091` が
blame の element を独立に prepaint する）。同じ形に直した。

この回帰は、位置ずれの原因究明のために `padding` を 0 と 7 で
撮り比べた**副産物として見つかった**。「フォントの em 幅差です」という
最初の説明を受け入れていたら、`padding=0` は一度も撮られず残っていた。
...
### 訂正（2026-08-09）: 3 と 4 は分離できない

上の着手順は 3（所属を設定引きに）と 4（`dockOnRight` の除去）を分けているが、
**分けられない。**

`projectPanel.dock` の既定は Zed と同じ `right`。所属が設定から決まる瞬間に
ファイルのパネルは**右ドックの所有**になる。ところが現在の描画は
「左ドックが所有し、`dockOnRight` で右に描く」形なので、
所有が右へ移った時点で `dockOnRight` の前提が崩れる。
左ドックは空になり、`dock(state, dockRight)` は
`raise newException(ValueError, "right dock has no panels yet")` に当たる。

**所有の移行と表示の移行は同じ 1 つの変更。** 3 と 4 を 1 ステップとして扱う。

### 分けられないなら、何で守るか

キャプチャの**全画素一致**で守る。

既定値（project_panel=right, terminal=bottom, agent=left）は
**今の画面の配置をそのまま再現する**。つまり正しく移行できたなら、
ウィンドウは 1 画素も変わらないはずである。

UI-118 の 1・2 でこの判定を実際に使い、直前のビルドと max diff 0 を確認した。
同じ判定を 3+4 にも使う。テストの緑では足りない（UI-113 / UI-117 の実例）。

### 1〜2 と 3+4 で UI-117 の 3 番目が解ける
...
**区切りの数が違う（Zed は 2 本、Nimculus は 1 本）。** 同じ見た目のアイコンが
別のものを指している。
...
| アイコン | 実装 | 何か |
| --- | --- | --- |
| `[▯]` | `status_bar.rs:232` `render_sidebar_toggle` | **スレッドサイドバー**のトグル。`ThreadsSidebarLeftClosed`。サイドバーが閉じていて左側のときだけ出る |
| 区切り 1 | `status_bar.rs:285-288` | サイドバートグルの直後（左側のとき）|
| `[✨]` | `left_dock_buttons` | **Agent パネル**のトグル。`default.json:1048,1059` で `"dock": "left"` |
| 区切り 2 | `dock.rs:1400-1402` | 左ドックのボタン群の直後 |
| `[🔍] [✓]` | `zed.rs:635,637` | 検索と診断 |

### 規則（2 つある）

**(1) ドックのトグルは、そのドックの位置と同じ側に出る。**
`workspace.rs:1757-1759`:
...
**(2) 区切りは、トグル群と他のステータス項目の境界に入る。**
`dock.rs:1392-1402`:

```rust
.when(has_buttons && (position == Bottom || position == Right),
      |this| this.child(Divider::vertical()))   // ボタンの前
.children(buttons)
.when(has_buttons && position == Left,
      |this| this.child(Divider::vertical()))   // ボタンの後
```

サイドバートグルも同じ形（`status_bar.rs:281-289`）。
右ドックのボタンは `dock.rs:1447` で `buttons.reverse()` される。

### Zed の既定のドック位置（`assets/settings/default.json`）

| パネル | 位置 | 行 |
| --- | --- | --- |
| agent | **left** | 1048, 1059 |
| project_panel | right | 797, 805 |
| outline_panel | right | 916, 922 |
| git_panel | right | 978, 982 |
| collaboration_panel | right | 970, 974 |
| terminal | **bottom** | 1834, 1851 |
| debugger | **bottom** | 2755, 2759 |

**左に出るトグルは Agent だけ。** 残りは全部右。

### Nimculus との差

| | Nimculus の位置 | Zed の規則が要求する位置 |
| --- | --- | --- |
| Agent `[✨]` | 区切りの**後ろ** | 左ドックのボタンなので区切りの**前** |
| 端末 | 左 | 下ドックなので**右** |
| ファイルのドックトグル `[▯]` | 左 | Zed の project_panel は右ドック |

Nimculus の `[▯]`（`Toggle Panel Dock`、`macos_platform.m:7803`）は
Zed の `[▯]`（スレッドサイドバー）と**別物**。見た目が似ているだけ。

### UI-097 の何が違ったか

UI-097 は「端末のトグルをパネルのトグルと左端で隣接させる」と決め、その理由を
**「Zed のステータスバー構成に視覚順を合わせるため」**と書いている。
合っていない。Zed の構成規則はドックの位置であって、
「パネル操作か、バッファの読み取り値か」ではない。
`macos_platform.m:7811-7814` のコメントは端末が下ドックだと**認識したうえで**
左に置いている。

**UI-097 の配置部分は本項で置き換える。** 他の内容（`commandPalette:toggle terminal`
の維持、28pt 行 / 24pt ヒット領域、セマンティックな境界色）はそのまま有効。

### 移植できるか

規則 (1)(2) は移植できる。`workspace_ui.nim:184-187` の `panelBelongsTo` が
既に `dockLeft` / `dockBottom` の所属を持っている。**情報はあり、使っていないだけ。**

ただし Nimculus には**右ドックが無い**（`workspace_ui.nim:78` は
`leftDock` と `bottomDock` のみ）。ファイルのドックは「左ドック」として
モデル化されつつ画面右に描かれている（`dockResizeDivider` の `dockOnRight`）。
**モデルと描画位置が食い違っている。** ここを直さずにトグルの側だけ動かすと、
規則ではなく個別対応になる。

### やること（この順）

1. 端末のトグルを右クラスタへ移し、その前に区切りを置く（下ドックの規則）
2. Agent のトグルを区切りの前へ移す（左ドックの規則）
3. ドックの所属からトグルの側を決める形にする（個別の分岐にしない）
...
パネルを増やすたびに同じ判断を手でやることになる。Zed は規則で書いている。却下。

**(b) UI-097 を尊重して現状維持。** UI-097 の根拠（「Zed に合わせる」）が
実測で否定された。根拠が消えた判断は維持しない。却下。

**(c) Zed のスレッドサイドバートグルも移植する。** これは
`MultiWorkspace` / スレッドサイドバーという Nimculus に無い機能のトグル。
移植対象は機能のほうが先。本項では扱わない。

### テスト観点

- 契約テスト: 下ドックのパネルのトグルが右クラスタに出ること、
  左ドックのそれが左クラスタで区切りの前に出ること、区切りの本数と位置
- キャプチャ: フッタ左端が `[✨] │ [🔍] [✓]` になること
...
**訂正（2026-08-10）: 下流ではなかった。** VM で測ると、
本文（Metal）もガター（AppKit）も同じ `#fafafa` を出す。
**2 経路で同じ 16 進値が違う画素になる、という前提が成り立たなかった。**
参照ディスプレイ固有の現象で、UI-111 に依存せずに直せた。
実際、テーマ表と焼き込み補正を直しただけで一致した。
...
### 再検討（2026-08-10）: 直す。判断の前提が変わった

UI-122 で `border` / `border.variant` に**同じ誤り**があり、
実測して直した結果、比が落ちることもなく解消した。
その過程で分かったこと:

**(1) 問題はテーマ表の値でもある。** `settings.nim:559-560`:

| 役割 | one.json | settings.nim |
| --- | --- | --- |
| `toolbar.background` | `#fafafa`（`:462`）| `#fcfcfc` |
| `tab.active_background` | `#fafafa`（`:465`）| `#fcfcfc` |
| `editor.background` | `#fafafa`（`:477`）| `#fcfcfc` |
| `editor.gutter.background` | `#fafafa`（`:478`）| `#fcfcfc` |

**焼き込み補正（`macos_platform.m:901`）だけの話ではない。**
テーマ表の 4 つの値が観測値になっている。
これは `border` と同じで、UI-111 に依存しない。

**(2) 権威は VM の実測であって参照ディスプレイではない。**
UI-112 は「補正を外すと参照ディスプレイでの一致が崩れる」を
据え置きの理由にした。しかし参照ディスプレイの観測値を正としたことが
罠 10 の誤りそのものだった。**Zed が塗る値が正**。

**(3) 実際に差として見えている。** タブバー帯（上端揃え）の残差 6.11% のうち、
罫線の色差を除いた分はこの `#fcfcfc` / `#fafafa`。
行 128–139 で、タブ面・ツールバー・本文のすべてが 2 ずれている。

### やること

1. テーマ表の 4 値を `#fafafa` に直す
2. `editorPaintToken()` の `#fafafa` → `#fcfcfc` 補正を**定義ごと削除**する
3. VM で測り、Zed の `#fafafa` と一致することを確認する

**1 だけやって 2 を残すと悪化する。** テーマが `#fafafa` を返し、
補正が `#fcfcfc` に変えるので、Metal の塗りは変わらないまま
AppKit 側だけが `#fafafa` になり、両者が食い違う。
UI-112 が記録した「2 経路の色管理差」が、今度は**値の差として**現れる。

### 残る懸念

補正のコメントは「AppKit オーバーレイにガターを別々に着色させるのでもなく」と
書いており、**ガターは AppKit、本文は Metal** という 2 経路の存在は変わらない。
色管理の差（同じ 16 進値が 2 経路で違う画素になる）が実在するなら、
値を揃えても VM で 1〜2 ずれる可能性がある。

**測ってから判断する。** ずれが残るなら、それは UI-111 の下流として
改めて記録する。ずれないなら、色管理差は参照ディスプレイ固有だったことになる。
...
| 要素モデル（即時 vs 保持） | **`UiTree` はそもそも描画に使われていない**。`demoTree` から `PaintList` を作る経路が無く（`grep` で 0 件）、用途は Accessibility ツリーの構築（main.nim:276）と AppKit ビューの矩形の追認（`:464-466`）だけ。ノード 10 個の影。即時／保持の選択は、要素ツリーが描画の源になって初めて意味を持つ |
...
**根拠の更新（2026-08-10）**: ここに書いていた `identical 73.59%` は
参照ディスプレイでの `tools/bitdiff.sh` の値で、
今日の測り方（テスト VM、上端／下端で揃える、`identical` を主指標）とは
比較できない。現在の実測は **identical 70.97%**（`4625ff1`）。

そのうえで結論は変わらない。**残差は行内のグリフの形と送り幅**で、
行グリッドは一致している（ピッチ 48 device px、開始 238 対 237）。
AppKit で描いていることが見た目の差として現れてはいない。

**さらに: UI-112 は UI-111 の下流ではなかった。**
「AppKit と Metal で同じ 16 進値が違う画素になる」という前提を VM で測ると、
両経路とも `#fafafa` を出した。この差は参照ディスプレイ固有で、
UI-111 に依存せずに直った。**下流と数えていた項目が 1 つ減った。**
