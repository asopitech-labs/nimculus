...
proc invalidateDemoUi() =
  ## All former synchronous composition call sites enter the window
  ## invalidator. The macOS setter is the existing native frame kick; the
  ## actual tree/paint/sidebar publication happens from receiveNativeFrame.
  demoWindow.markLayoutDirty(demoSidebarNode, sidebar = true)
  when defined(macosx):
    platformSetEditorInputPane(uint32(if editorSession.split:
      editorSession.splitActivePane else: 0))
when defined(macosx):
...
proc setupDemoUi() =
  ...
  demoWindow.beginDraw()
  let document = activeDocument()
  let hasDocument = document != nil
  syncWorkspaceUiTabs()
  demoTree = newUiTree()
  demoTree.onLayoutDirty = proc(id: NodeId) =
    demoWindow.markLayoutDirty(id, sidebar = id == demoSidebarNode)
  ...
  demoSidebarNode = sidebar.node
  demoWindow.registerSidebarView(demoSidebarNode)
  ...
  demoTree.setContext(root, parseKeyContext("Workspace"))
  demoTree.setContext(toolbarControl.node, parseKeyContext("Toolbar"))
  demoTree.setContext(button.node, parseKeyContext("Button"))
  demoTree.setContext(scroll.node, parseKeyContext("Editor"))
  demoTree.setContext(tabs.node, parseKeyContext("TabBar"))
  demoTree.setContext(activeTab.node, parseKeyContext("Tab"))
  demoTree.setContext(sidebar.node, parseKeyContext("Sidebar"))
  demoTree.setContext(projectRow.node, parseKeyContext("Project"))
  demoTree.setContext(statusBar.node, parseKeyContext("StatusBar"))
  demoTree.setContext(statusItem.node, parseKeyContext("StatusItem"))
...
proc renderDemoUi() =
  ...
  ## every invocation through the frame boundary.
  let renderer = setupDemoUi
  renderer()
