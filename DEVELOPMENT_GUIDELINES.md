...
- Git、LSP、検索、タスクは UI スレッドをブロックしない。同期APIを先に提供する場合も、UIからは非同期サービスへ接続し、キャンセル・進捗・完了通知を明示する
- 外部プロセスの stdout/stderr は `readAll` で無制限に保持しない。非ブロッキングで消費し、機能ごとの上限、UTF-8境界、切り詰め状態を定義する。diff表示はZedのように表示用圧縮・上限化を行う
...
ターミナル、タスク、LSP、検索など外部プロセスの出力は無制限にメモリへ蓄積しない。出力上限、UTF-8境界、行境界での切り詰め、切り詰め発生の表示または記録を実装し、上限付近と長時間実行をテストする。
...
拡張の第 1 段階は language definition、Tree-sitter grammar、LSP configuration、theme、icon theme、snippets、tasks、commands とする。macOSの第 2 段階では、manifest/API検証後に公式Wasmtime CLIを直接argvで起動するWASM実行境界を使い、extension rootだけをWASI preopenする。external processはprocess権限を確認し、シェルを介さず直接argv・extension root cwd・Task出力・cancelへ接続する。HTTPS catalogはTask境界で同期し、SHA-256検証済みZIPを隔離展開してからmanifest権限確認と原子的installを行う。第 3 段階では、ZedのWasmHost/WIT境界を参照し、任意のWasmtime開発環境へリンクせず動的に解決するin-process Component Model/WASI実行層を追加する。この層は現在、WASI Preview 2、extension rootのpreopen、API/id/host capability環境、fuel・resource limit、明示的な引数なしexport呼び出し、専用worker、idle polling、epoch interruptionによるキャンセルまでをmacOSで実装している。最初の実WIT host importはZedと同じ`zed:extension/platform.current-platform`であり、未実装importは暗黙に許可せず決定的なtrapへ落とす。UIスレッドからWasmtimeを同期呼び出しせず、core moduleはCLIへフォールバックする。manifest権限はallow-listで検証し、filesystem-write/process/networkなど追加権限を実行・導入前の非同期macOS Allow/Denyシートで確認する。Zedの全WIT world互換を一度に導入せず、実際にリンクするhost capabilityをversioned APIとして段階的に追加する。互換アーキテクチャのWasmtime Cライブラリがない環境では、Component実行を利用可能と報告せずCLI境界を使用する。
...
Nimble の各テスト・ベンチマークは専用の `--nimcache` を使用し、テスト間の
生成物を共有しない。共有キャッシュの破損や異なるコンパイル条件による
再現性低下を、品質ゲートの失敗として扱う。

### 10.3 性能計測

次の指標を計測可能にする。

`cold start`、`idle memory`、`input latency`、`frame time`、`layout time`、`text shaping`、`workspace load`、`file watcher load`、`LSP memory`、`terminal memory`、`remote latency`、`allocation count`をTSVで記録する。アプリのidle境界でresident memory、live allocation、frame/inputを定期記録する`benchmark_soak.sh`を使い、macOS CIでは20秒のidle soak smokeを回帰ゲートにする。既定8時間の実機計測では完了行を確認する。計測ハーネスを追加しただけでは8時間連続利用の完了と扱わない。

AppKit/Metalの実アプリスモークは、macOSのログイン済みGUIセッションかmacOS CI runnerで実行する。Codexなどのサンドボックス内から`NSApplication sharedApplication`を直接起動すると、アプリコードの不具合ではなくmacOSの`_RegisterApplication`で`SIGABRT`になることがあるため、その場合はフレームゲートを弱めず、サンドボックス外の同一コマンドで再検証する。検証結果には実行環境と`frames=`を必ず残す。

目標は、通常起動 1 秒未満、空ワークスペース 50〜100MB 以内、100MB 級ファイル、10 万ファイル級ワークスペース、8 時間連続利用、長時間アイドルでメモリが増加し続けないこととする。

### 10.4 ビルドキャッシュとディスク容量

ビルド・テスト・ベンチマークの実行前後に、必ず次を確認する。

- `df -h / /tmp` で空き容量を確認する
- workspace の `.nimcache/`、`nimcache/`、`nimblecache/`、`dist/`、テスト生成バイナリを確認する
- `~/.cache/nim` と一時ディレクトリの Nimculus / Nim cache を確認する
- `git status --short` で生成物が追跡対象になっていないことを確認する

各検証は専用の`--nimcache:/tmp/nimculus-<目的>-cache`を使い、検証後に不要な一時cacheと生成バイナリを削除する。通常の`nimble build`も`.nimcache/build`へ固定し、`nimble clean`でworkspace内の再生成可能なcacheと配布生成物を整理できるようにする。workspace内のcacheが500MBを超えた場合、またはディスク空き容量が20%未満になった場合は、次の実装へ進む前にcacheを整理する。`.gitignore`で除外されていることだけを理由に放置しない。削除しても再生成可能なビルドcacheのみを対象とし、参照リポジトリやユーザーのソース・設定は削除しない。

### 10.5 ターミナルに配慮した検証出力

コンパイル、テスト、ベンチマーク、GUIスモークの標準出力・標準エラーを、長時間にわたり端末へ直接流し続けない。特にNimのコンパイルログや反復テストは一時ログへリダイレクトし、成功時は要約、失敗時は末尾の限定行と終了コードだけを表示する。ログ本体は `/tmp/nimculus-<目的>.log` に保存し、不要になったら削除する。

AppKit / Metal を起動する検証は短時間の独立プロセスとして実行し、専用の `HOME`、専用の `--nimcache`、明示的なタイムアウトを使用する。端末の描画負荷を検証手順の一部にしない。クラッシュ時はアプリの終了コード、DiagnosticReports、実行環境を確認してから再実行し、フレーム数や検証条件を弱めて成功扱いにしない。
