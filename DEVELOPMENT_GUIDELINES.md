...
各検証は専用の`--nimcache:/tmp/nimculus-<目的>-cache`を使い、検証後に不要な一時cacheと生成バイナリを削除する。workspace内のcacheが500MBを超えた場合、またはディスク空き容量が20%未満になった場合は、次の実装へ進む前にcacheを整理する。`.gitignore`で除外されていることだけを理由に放置しない。削除しても再生成可能なビルドcacheのみを対象とし、参照リポジトリやユーザーのソース・設定は削除しない。
